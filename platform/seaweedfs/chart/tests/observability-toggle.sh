#!/usr/bin/env bash
# bp-seaweedfs observability-toggle integration test
# (docs/BLUEPRINT-AUTHORING.md §11.2).
#
# Verifies:
#   - default `helm template` produces zero monitoring.coreos.com/v1
#     resources;
#   - opt-in render with seaweedfsOverlay.serviceMonitor.enabled=true
...
helm template smoke-seaweedfs . > "$TMP/default.yaml"
if grep -q "monitoring.coreos.com" "$TMP/default.yaml"; then
  echo "FAIL: default render of bp-seaweedfs contains monitoring.coreos.com references." >&2
...
if grep -q "kind: ServiceMonitor" "$TMP/default.yaml"; then
  echo "FAIL: default render of bp-seaweedfs contains kind: ServiceMonitor." >&2
  exit 1
fi
echo "  PASS"

# ── Case 2: opt-in (overlay serviceMonitor.enabled=true) renders cleanly ─
echo "[observability-toggle] Case 2: opt-in (seaweedfsOverlay.serviceMonitor.enabled=true) renders cleanly"
if ! helm template smoke-seaweedfs . \
    --api-versions monitoring.coreos.com/v1 \
    --set seaweedfsOverlay.serviceMonitor.enabled=true \
...
if ! helm template smoke-seaweedfs . \
    --set seaweedfsOverlay.serviceMonitor.enabled=false \
...
echo "[observability-toggle] All bp-seaweedfs observability-toggle gates green."
