...
#      it via the configure-oauth Job, NOT via existingSecret).
#   2. The configure-oauth Job has OIDC_EXTRA_REDIRECT_PARMS env
...
#   3. The Job payload posts `oidc_extra_redirect_parms` to Harbor's
...
echo "[g117-5-sso-tier1] Case 2: configure-oauth Job has OIDC_EXTRA_REDIRECT_PARMS env"
if ! grep -q 'name: OIDC_EXTRA_REDIRECT_PARMS' "${tmpdir}/default.yaml"; then
  echo "FAIL: OIDC_EXTRA_REDIRECT_PARMS env missing from configure-oauth Job" >&2; exit 1
fi
...
echo "[g117-5-sso-tier1] Case 3: Job payload includes oidc_extra_redirect_parms field"
if ! grep -q 'oidc_extra_redirect_parms' "${tmpdir}/default.yaml"; then
  echo "FAIL: Job payload does not include oidc_extra_redirect_parms" >&2; exit 1
fi
echo "  PASS"
