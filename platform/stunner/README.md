...
K8s-native TURN/STUN for WebRTC NAT traversal. **Application Blueprint** (see [`docs/ARCHITECTURE.md`](../../docs/ARCHITECTURE.md) §4.5 — Communication). Used by `bp-relay` to make LiveKit (WebRTC SFU) reachable from clients behind NATs.
...
- `chart/templates/servicemonitor.yaml` — `monitoring.coreos.com/v1.ServiceMonitor` (DEFAULT FALSE per [`docs/RUNBOOKS.md`](../../docs/RUNBOOKS.md) §11.2; double-gated on Capabilities).
