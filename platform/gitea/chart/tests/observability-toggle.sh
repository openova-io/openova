#!/usr/bin/env bash
# bp-gitea observability-toggle integration test (issue #182).
#
# Verifies docs/BLUEPRINT-AUTHORING.md §11.2 — the upstream gitea
# chart's `gitea.metrics.serviceMonitor.enabled` AND the bundled
# postgresql subchart's `metrics.serviceMonitor.enabled` /
...
helm template smoke-gitea . > "$TMP/default.yaml"
if grep -q "monitoring.coreos.com" "$TMP/default.yaml"; then
  echo "FAIL: default render of bp-gitea contains monitoring.coreos.com references." >&2
...
if grep -qE "kind: (ServiceMonitor|PodMonitor|PrometheusRule)" "$TMP/default.yaml"; then
  echo "FAIL: default render of bp-gitea contains a Prometheus operator resource." >&2
  exit 1
fi
echo "  PASS"

echo "[observability-toggle] Case 2: opt-in (gitea.metrics+serviceMonitor=true) produces a ServiceMonitor"
if ! helm template smoke-gitea . \
    --set gitea.gitea.metrics.enabled=true \
    --set gitea.gitea.metrics.serviceMonitor.enabled=true \
...
if ! helm template smoke-gitea . \
    ...
    --set gitea.postgresql.metrics.enabled=false \
    --set gitea.postgresql.metrics.serviceMonitor.enabled=false \
    --set gitea.postgresql.metrics.prometheusRule.enabled=false \
...
echo "[observability-toggle] All bp-gitea observability-toggle gates green."
