...
| 4. Bootstrap-kit install | `clusters/<sovereign-fqdn>/` (Flux-reconciled) | Flux installs the **15 G2 wrapper Helm charts** (each a `bp-<name>:<semver>` OCI artifact published by [`.github/workflows/blueprint-release.yaml`](../.github/workflows/blueprint-release.yaml)) in dependency order: **bp-cilium → bp-cert-manager → bp-flux** (host-level reconciler for the cluster's own Kustomizations) **→ bp-crossplane → bp-spire** (server + agent) **→ bp-nats-jetstream → bp-openbao** (3-node Raft, auto-unseal seeded by cloud-init per #311) **→ bp-external-secrets** (ESO + ClusterSecretStore — sole secret pipeline post-#194 Option 2) **→ bp-keycloak** (embedded postgresql; per topology choice) **→ bp-gitea** (embedded postgresql; with public Blueprint mirror) **→ bp-cnpg** (CloudNativePG operator — required by bp-powerdns) **→ bp-powerdns** (per-Sovereign authoritative zone with `pdns-pg` CNPG cluster, #167) **→ bp-external-dns** (writes per-Sovereign A/CNAME records into bp-powerdns) **→ bp-crossplane-claims** (XRDs/Compositions split from bp-crossplane controller for CRD-ordering correctness) **→ bp-catalyst-platform** (umbrella; trigger event for handover to the new Sovereign's console). All 19 Day-1 charts (bp-valkey, bp-seaweedfs, bp-harbor, bp-opentelemetry/alloy/loki/mimir/tempo/grafana, bp-langfuse, bp-kyverno, bp-reloader, bp-vpa, bp-trivy, bp-falco, bp-sigstore, bp-syft-grype, bp-velero, bp-coraza) are NOT in Phase 0 — they are discoverable as marketplace items the new sovereign-admin installs post-handover (see §5). |
| 5. Crossplane adoption | Crossplane Compositions in `clusters/<sovereign-fqdn>/` | Crossplane adopts management of all infrastructure created by OpenTofu in step 2; ESO + OpenBao is the day-2 secret distribution path from t=0 (no sealed-secrets — removed per #194 Option 2, founder verdict 2026-04-30); further DNS records (gitea/admin/api/harbor) are written by `external-dns` against the per-Sovereign PowerDNS zone via the PowerDNS REST API (NOT against the registrar). Phase 1 begins (see §4). |
...
**Implementation status:** the Go wrapper, OpenTofu module, and 15 G2 wrapper charts (the cascade above) all exist today (verified at [`IMPLEMENTATION-STATUS.md`](IMPLEMENTATION-STATUS.md) §7). The pool-domain-manager (`core/pool-domain-manager/`) and its 5 registrar adapters are deployed and running in `openova-system`. End-to-end DoD against a real Hetzner project is pending Group M of the [Catalyst-Zero Provisioning Plan](PROVISIONING-PLAN.md). Per #310 (chart trim) the bootstrap-kit kustomization is being restored to this 15-chart Phase 0 minimum after a period of drift to 35 entries; #194 Option 2 removed bp-sealed-secrets entirely.
