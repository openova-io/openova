...
| `products/catalyst/` umbrella (`bp-catalyst-platform`) | 🟦 | Has `bootstrap/{ui,api}/` (React SPA wizard + Go bootstrap API) + `chart/` with Chart.yaml + Helm templates for the full Catalyst-Zero deployment + `crds/` with the 12 CRDs. Canonical Helm chart for Catalyst-Zero and every franchised Sovereign. |
...
|---|---|---|
| console (Catalyst UI) | 🚧 | Astro + Svelte UI at `core/console/`. Deployed on Catalyst-Zero. BSS menu (vouchers, billing, catalog, orders, tenants) now lives inside the operator console at `/bss` — replaces the legacy `admin.<sovereign-fqdn>` URL. |
...
| admin (sovereign-admin operations UI) | 🚧 | Astro + Svelte UI at `core/admin/`. Voucher / billing / catalog / orders / tenants admin surface — being folded into console `/bss` per BSS migration. |
...
| catalyst-api | 🟦 | Go bootstrap API at `products/catalyst/bootstrap/api/`. **CODE-COMPLETE** Pillar 3 install path: provisioning now generalises `bp-cnpg-pair` install beyond WP-only (PR [#2073](https://github.com/openova-io/openova/pull/2073)); threads `Tenant.AppConfigs` into rendered manifests (PR [#2053](https://github.com/openova-io/openova/pull/2053), TBD-V27); emits a `Continuum` CR for each multi-region tenant app (PR [#2074](https://github.com/openova-io/openova/pull/2074)). |
...
| `Organization` | 🚧 | Schema in `organization.yaml` (slice B1, PR #1106). organization-controller is slice C1 — not yet built. |
| `Environment` | 🚧 | Schema in `environment.yaml` (slice B2, PR #1107). environment-controller is slice C2 — not yet built. |
| `Application` | 🚧 | Schema in `application.yaml` (slice B3, PR #1105). application-controller is slice C4 — not yet built. |
| `Blueprint` | 🚧 | Schema in `blueprint.yaml` (slice B4, PR #1112). Serves `v1alpha1` (legacy) + `v1` (canonical). All 59 existing platform/products blueprint.yamls validate. blueprint-controller is slice C3 — not yet built. |
...
Go types live at `core/pkg/apis/v1alpha1/` (placeholder `.gitkeep`); Go types are added when control-plane services are scaffolded (slice C1..C5 of #1095).
