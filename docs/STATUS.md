...
**Status:** Authoritative. Living document. **Updated:** 2026-08-26 (re-stamped to hw305 truth: Huawei kom4dc substrate, 11-step cutover chain, Pillar-4 = Agenity + `bp-openova-mcp`, UAT ledger 286 rows / ✅246·❌12·⚠️6·⏳22 after PR #6690; live cutover-keystone + per-Sovereign env inventory still to be re-derived from the deployments API). **Staleness rule:** if this stamp is more than 14 days old, treat this file as drifted — re-derive live state before trusting any row (see §11 Re-stamp cadence).
...
| Catalyst BSS (chargeback) | ✅ | `products/chargeback` (`bp-chargeback`, ADR-0014, EPICs #6723 → #6867; product name **Catalyst BSS** since 2026-09-11, identifiers rename together at 0.2.0): **live on hw307 at chart 0.1.49 / image 0.1.47 (2026-09-20 15:17Z)**. Capacity (DESIGN.md §11, rebuilt 2026-09-20): four tabs (Pools · Placements · Shapes · Regions & zones); a pool lists the classes it can ENFORCE (guaranteed and spot everywhere — spot is a cheaper price plus the right to delete — burstable only where the host throttles at runtime), one SKU or family sits on a pool once per class, each running resource counts at its override, else its `lifecycle` tag, else the most cautious placed class, a guaranteed floor per resource keeps burstable out of guaranteed room, and a reclaim proposal names which spot to give back, newest first (BSS says how much and which; the platform deletes). Every SKU, family, resource, pool and class is chosen from a list. Partners: a tier and a partner can be deleted, refused by name while anything depends on them; permanent records (issued statements, ledger entries) mean suspend, not delete. All of it walked on hw307 by clicking (#6942, #6943, #6944; #6936 and #6937 closed with live evidence). Metering: Huawei cloud collector (15-min inventory + CTS events + hourly CES; Elastic IP billed on reservation OR traffic from the bandwidth charge mode; in-place resize split at the minute) and the platform collector (Organizations, plan-hours, k8s meters). Rating: price books scoped cloud/platform and assigned per source, committed plans vs pay-per-use on disjoint meters, discounts with a stated combination rule, reporting currency + rates, 5 % tax with per-customer profile and a frozen tax snapshot. Billing: four-field commercial profile (charging, payment model, method, gateway), gapless invoices with PO/terms/due date and a store-enforced lifecycle, payments with part payment and settlement at the currency minor unit, account ledger and per-customer balance, top-up, apply-credit, allocation across invoices, refunds, credit notes (gapless CN numbering), collections (aging, reminder schedule, escalation, suspend/resume on command). Integration: billing-system-of-record seam (internal/external, TMF-shaped outbox, CSV exporter, signed status/payment/balance/enforcement imports, summary-charge variant) and the payment-gateway seam (Stripe registered; signed gateway callback route). Synthetic three-month showcase history plus a converging landlord backfill on the traffic model. Enforcement is live end to end: chargeback suspends and resumes an Organization at the platform through an internal ServiceAccount-authenticated route (walked on hw307). Access control is live: six roles at two scopes (sovereign, customer), role bindings, directory group mapping from the SSO gate, permission-gated console, `/api/v1/me`; the signed gateway callback books payments with the callback secret alone; the host-namespace quota is the purchased plan plus the vCluster control plane plus the per-Organization platform stack (Keycloak, newapi, openclaw, Agenity), all three derived from rendered values and pinned by tests; the platform collector excludes those pods from customer usage and keeps them on the overhead line. Known follow-ups: a plan-change route on the sovereign-admin API (today only the CR carries a plan change), Agenity PVC labels for exclusion, the per-Organization Anthropic credential is operator-supplied. Waiting on Omantel's specification: their gateway and billing adapters. Evidence: PRs #6903, #6907, #6908 and the #6867 comments. |
...
| Test/walk Sovereigns (`hw<NNN>` series) | 🚧 | Convention: `hw<NNN>.omani.works` ↔ `hw<NNN>.omantel.biz` (TLD rotation when LE-rate-limited), provisioned on **Huawei kom4dc** — the delivery substrate: 2-region `me-east-215-a`/`-b`, one VPC per region (quota 5 total), EVS 400-volume cap, ~15-min wipe-release lag ([`docs/runbooks/preflight-sovereign-provision.md`](runbooks/preflight-sovereign-provision.md)). Fired per release-train, never for one passenger. Latest walked env = the highest `uat-hw<NNN>` branch (hw305 as of 2026-08-26; furthest cutover ever = hw246). Forbidden test domains unchanged: `openova.io`, `omantel.openova.io`, `Nova Cloud`, `eventforge.io`. |
| `omantel` production Sovereign (`omantel.biz`) | ✅ | **PERMANENT production Sovereign, live on Huawei kom4dc** — current deployment env is stamp-dependent (re-verify against the live deployments API before relying on the name; production was destroyed twice by in-band quota reclaim — #4614, #4675 dep 2c3f7c34). On the never-touch protect-list: never a wipe target, never a cutover re-fire target, never an in-band reclaim victim. (The only other founder-protected shared resource is the bastion node `bastion-openova`, EIP 212.72.24.20.) |
...
| Bootstrap kit (cilium → cert-manager → flux → crossplane → sealed-secrets → nats-jetstream → openbao → keycloak → gitea → powerdns → bp-catalyst-platform) | 🚧 | G2 wrapper Helm charts exist under `platform/<x>/chart/`. Each carries blueprint.yaml, values.yaml, Chart.yaml, published as `bp-<name>:<semver>` OCI artifact. `platform/spire/` retained as opt-in but NOT in the bootstrap chain (PR #665, 2026-05-03). Steady-state DoD pending real Huawei kom4dc provisioning (Group M). |
...
Live per-row walk state lives in [`docs/ledger/UAT.md`](ledger/UAT.md) (~286 rows, all row-ID formats counted; live tally ✅246·❌12·⚠️6·⏳22 on hw305 as of 2026-08-25 after PR #6690 — point-in-time, defer to the ledger head) and [`docs/ledger/TRUST.md`](ledger/TRUST.md). The "Anchoring evidence" column above records the code-complete genealogy — the ledgers, not this table, carry the current walk stamps.
