...
## 11. Secret rotation cadence + procedures
...
### 11.2 GHCR pull token (`catalyst-ghcr-pull-token`)
...
**Why this token has its own runbook.** The bootstrap-kit pulls the bp-* OCI artifacts from `ghcr.io/openova-io/`, which is a **private** registry path. Without the token, the source-controller logs `failed to get authentication secret 'flux-system/ghcr-pull': secrets "ghcr-pull" not found` and Phase 1 stalls at bp-cilium.

**Where the token must NEVER be:** git (any branch, any repo), the bootstrap-kit YAMLs, the catalyst-api Pod logs, the Hetzner project metadata, Slack/email/issue bodies.

**Generation.** Fine-grained PAT at https://github.com/settings/personal-access-tokens/new — Resource owner: openova-io; Repository access: Public Repositories (read-only); Permissions: Account → Packages → Read; Expiration: 365 days. Copy to 1Password immediately.

**Storage.** 1Password vault: **OpenOva — Production**. Item title: **Catalyst — GHCR pull token (catalyst-ghcr-pull-token)**. Tags: `catalyst`, `ghcr`, `rotation:yearly`.

**Apply (the one-liner).** Replace `<GHCR_PULL_TOKEN>` with the token retrieved from 1Password — never paste a real token into git, an issue, or a transcribed terminal:
...
**Rollback.** If the new token does not authenticate, restore the previous token via 1Password version history + re-run the kubectl one-liner + rollout. The previous token remains valid until next yearly rotation — GitHub does not invalidate replaced fine-grained tokens automatically. Revoke the broken token in the GitHub UI as a hygiene step once rollback succeeds.

### 11.3 Hetzner Cloud API token (per Sovereign)
...
Rotation: per-Sovereign apply. Each `tofu apply` accepts a fresh token; once `tofu apply` returns, catalyst-api drops the value out of memory.
...
### 11.4 Dynadot API key + secret (`dynadot-api-credentials`)
...
2. **NEVER commit a credential.** Use this runbook's `kubectl create secret … | kubectl apply` one-liner; the value never touches a file the working tree tracks.
3. **NEVER skip the rollout restart.** `secretKeyRef` env vars are read at Pod start. A Secret update with no rollout is a silent half-rotation: existing Pods serve the old value, new Pods (post next evict) serve the new one.
