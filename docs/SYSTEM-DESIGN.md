...
| **Organization** | the multi-tenancy unit | `Organization` CR (`orgs.openova.io`) + a dedicated vCluster on every plan (since 2026-09-10 no plan shares the bare host namespace). The host-namespace ResourceQuota `plan-quota` is the **purchased plan plus the vCluster control-plane overhead**: plan S 2 vCPU / 4 GiB, M 4 / 8, L 8 / 16, XL 16 / 32, each **+ 520m CPU / 1088Mi on requests and + 1500m / 1194Mi on limits** for the `vcluster-0` syncer (500m / 1Gi) and the synced coredns (20m / 64Mi requests, 1000m / 170Mi limits) that share the namespace, plus their 5Gi backing-store volume — so the control plane never eats into what the customer bought (S renders `requests.cpu: 2520m`, `limits.cpu: 3500m`). Flexi has no hard quota; the LimitRange per-container defaults are plan-only. The split is stamped on the object as `openova.io/plan-cap` / `openova.io/vcluster-control-plane-overhead` annotations, and chargeback excludes the control-plane pods from the `k8s.*` meters (`core/controllers/organization/internal/gitops/manifests.go` `vclusterControlPlaneOverhead`) |
