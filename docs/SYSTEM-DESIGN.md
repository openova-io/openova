...
| **Organization** | the multi-tenancy unit | `Organization` CR (`orgs.openova.io`) + a dedicated vCluster on every plan (since 2026-09-10 no plan shares the bare host namespace). The host-namespace ResourceQuota `plan-quota` is the **purchased plan plus the vCluster control-plane overhead plus the per-Organization platform-stack overhead**: plan S 2 vCPU / 4 GiB, M 4 / 8, L 8 / 16, XL 16 / 32, each **+ 520m CPU / 1088Mi on requests and + 1500m / 1194Mi on limits** for the `vcluster-0` syncer (500m / 1Gi) and the synced coredns (20m / 64Mi requests, 1000m / 170Mi limits) that share the namespace, plus their 5Gi backing-store volume, and each **+ 3840m / 6064Mi on requests and + 4550m / 7168Mi on limits** (XL: + 4835m / 8096Mi and + 5500m / 9152Mi) for the platform stack every Organization is delivered with — `bp-keycloak` (1 / 2Gi) and its postgresql (500m / 512Mi), the `bp-newapi` pod (newapi 500m / 256Mi → 500m / 1Gi + its sandbox-bridge and metering sidecars) and its CNPG postgresql (500m / 512Mi), the `bp-openclaw` controller (250m / 512Mi), `bp-agenity` (1 / 2Gi + its creds-resync sidecar; on XL the 2-CPU / 4Gi LimitRange default sizes its unsized init container above them) and its oidc-gate (50m / 64Mi) — so neither overhead eats into what the customer bought and the customer's catalog applications consume the plan (S renders `requests.cpu: 6360m`, `limits.cpu: 8050m`). Flexi has no hard quota; the LimitRange per-container defaults are plan-only. The split is stamped on the object as `openova.io/plan-cap` / `openova.io/vcluster-control-plane-overhead` / `openova.io/platform-stack-overhead` annotations (`openova.io/quota-formula`), and chargeback excludes both the control-plane pods and the platform-stack pods from the customer's `k8s.*` meters (`core/controllers/organization/internal/gitops/manifests.go` `vclusterControlPlaneOverhead`, `platformStack`) |
