...
### Reconcile Pass 3 — 2026-04-29 (drift fixed)

**Date:** 2026-04-29. **Branch:** `docs/reconcile-pass-3`. **Base:** main @ `934e519b` (post-Reconcile-Pass-2 batch — the admin landing surface replaces the DAG provision view at `/sovereign/provision/$deploymentId` with the application card grid + per-app tabs `4047ba1d`/`a75414a7`; all 11 bp-* charts converted to umbrella v1.1.0 with proper upstream subchart payloads `43aff202`/`e42799fa`; catalyst-api per-component HelmRelease watch via new `internal/helmwatch/` package emitting `phase: "component"` SSE events `5be6bcba`; deployments persisted to RWO PVC `catalyst-api-deployments` 1Gi via new `internal/store/` `418cead0`; cloud-init flux-bootstrap split into `bootstrap-kit` + `infrastructure-config` Kustomizations `34c8de84`/`2da4c43c`; Cilium installed BEFORE Flux in cloud-init with `k8sServiceHost=127.0.0.1` `e571ec7a`/`54872009`; bp-* HelmRepository url corrected to `oci://ghcr.io/openova-io` + `secretRef: ghcr-pull` `efa41803`; kube-system Namespace dropped from `01-cilium.yaml` + `05-sealed-secrets.yaml` `2022e1af`; OpenTofu CLI v1.11.6 + `infra/hetzner/` module bundled into the catalyst-api image with SHA256 verification `9b6c297d`/`61c61226`; `null_resource.dns_pool` removed from `infra/hetzner/main.tf` `330211d2`; CPX SKU family accepted in tofu validation regex + empty `worker_size` allowed for solo Sovereigns `c6cbfe68`; Containerfile build stage bumped go 1.23 → 1.26 to match `go.mod` `586f0dc2`; blueprint-release CI hollow-chart guards verifying upstream subchart presence at every step `54418bd5`/`35dcb84d`/`bdeb0f54`). **Skill:** `reconcile-catalyst-docs full`. **Trigger:** post-major-architectural-changes batch. **Verdict:** ✅ drift fixed — canonical docs realigned with the admin app-card landing surface (DAG retired), the umbrella v1.1.0 OCI artefacts (v1.0.0 / v1.0.1 hollow), the per-component HelmRelease watch + SSE phase shape, the deployments-PVC persistence model, the bundled OpenTofu CLI + `infra/hetzner/` module, the Cilium-before-Flux cloud-init ordering, the bootstrap-kit + infrastructure-config Kustomization split, the `oci://ghcr.io/openova-io` HelmRepository url + ghcr-pull secretRef correction, and the CPX-family-allowed + empty-worker-size tofu validation regex.

**Ground truth at HEAD `934e519b`:**

- 56 platform components (`ls platform/ | wc -l = 56`) — anchor unchanged from Pass 1/2.
- 6 products (axon, catalyst, cortex, fabric, fingate, relay).
- 11 platform `blueprint.yaml` files + 14 Helm `Chart.yaml` files unchanged. All 11 bp-* charts are umbrella v1.1.0 (each declares its upstream chart under `dependencies:` per `BLUEPRINT-AUTHORING.md` §11.1; the historical v1.0.0 / v1.0.1 OCI artefacts are HOLLOW and broken).
- Live in `openova-system` on Catalyst-Zero: `pool-domain-manager`, `powerdns` (`bp-powerdns:1.1.0+ef3c785bfd24`, verified via `kubectl get helmrelease -A`), `dnsdist`, CNPG-managed `pdm-pg` + `pdns-pg`. Live in `catalyst`: catalyst-api (`CATALYST_TOFU_WORKDIR=/tmp/catalyst/tofu` + `CATALYST_DEPLOYMENTS_DIR=/var/lib/catalyst/deployments` envs, 1 Gi memory limit, RWO PVC `catalyst-api-deployments` 1Gi mount with `fsGroup: 65534`, Recreate strategy) + catalyst-ui.
- Wizard step ordering canonical from `STEPS` in `src/pages/wizard/WizardPage.tsx` is **Org → Topology → Provider → Credentials → Components → Domain → Review** (unchanged from Pass 2).
- Sovereign Admin landing surface lives at `/sovereign/provision/$deploymentId` (`AdminPage.tsx`); per-Application page at `/sovereign/provision/$deploymentId/app/$componentId` (`ApplicationPage.tsx`) with Overview / Logs / Dependencies / Status tabs. The legacy `ProvisionPage.tsx` is a 22-line re-export shim. The DAG view (1300 lines of SVG bubbles + edges + supernode mapping) was gutted at `4047ba1d`.
- Cluster manifests at `clusters/_template/bootstrap-kit/01-cilium.yaml … 11-bp-catalyst-platform.yaml` reference OCI artefacts at `oci://ghcr.io/openova-io` with `secretRef: ghcr-pull`, all chart `version: 1.1.0` (`grep version: clusters/_template/bootstrap-kit/*.yaml` returns 11 lines all at 1.1.0).
- catalyst-api per-component watch: `internal/helmwatch/{helmwatch,kubeconfig,logtailer}.go` exists; emits SSE events `phase: "component", component: <id>, state: pending|installing|installed|failed|degraded`. Result struct gains `ComponentStates map[string]string` + `Phase1FinishedAt *time.Time` + `Kubeconfig string`.
- New endpoints: `GET /api/v1/deployments/<id>/kubeconfig` and `GET /api/v1/deployments/<id>/events`.
- `infra/hetzner/main.tf` no longer carries `null_resource.dns_pool` (PDM is the single owner of pool-domain Dynadot writes); `variables.tf` regex `^(cx[0-9]+|cpx[0-9]+|ccx[0-9]+|cax[0-9]+)$` accepts every Hetzner SKU family + `worker_size = ""` valid for solo Sovereigns. Default `control_plane_size = cx42` (16 GB Intel; `cpx32` 8 GB AMD also recommended); default `worker_size = cx32` with `worker_count = 0` for solo.
- catalyst-api Containerfile bundles the OpenTofu CLI v1.11.6 (SHA256-verified for amd64 + arm64) and the `infra/hetzner/` module under `/infra/hetzner/` so the catalyst-api Pod IS the OpenTofu runner — no host-side install required. Containerfile build-stage Go bumped 1.23 → 1.26 to match `go.mod` (`586f0dc2`).
- Cloud-init now installs k3s with `--flannel-backend=none`, then **Cilium first via Helm** with `k8sServiceHost=127.0.0.1` (breaks the CNI bootstrap deadlock that previously needed the LB IP that didn't exist yet), then Flux core, then a single manifest creating the GitRepository + two Kustomizations (`bootstrap-kit` for HelmReleases, `infrastructure-config` for Provider package + ProviderConfig + Compositions, `dependsOn: bootstrap-kit` + `wait: true`).
- `01-cilium.yaml` and `05-sealed-secrets.yaml` no longer carry duplicate `kube-system` Namespace declarations (kustomize built-in namespace, never re-declare).
- blueprint-release CI runs four hollow-chart guards on every publish (after `helm dependency build`, after `helm package`, after `helm push`, plus a `helm template` smoke render — `54418bd5`/`35dcb84d`/`bdeb0f54`).

**Drift identified and fixed (six categories per skill spec):**

A. **Numerical drift (3):**
- `IMPLEMENTATION-STATUS.md` L78 claimed `bp-powerdns:1.0.6`; actual chart at `platform/powerdns/chart/Chart.yaml` is **1.1.0** and live HelmRelease confirms `bp-powerdns@1.1.0+ef3c785bfd24`. Updated.
- `PLATFORM-POWERDNS.md` L190, L209, L235 all referenced `bp-powerdns:1.0.6`. Updated to `1.1.0` (3 places via `replace_all`).
- `ORCHESTRATOR-STATE.md` L41 claimed "bp-powerdns at 1.0.6". Updated to call out v1.1.0 + the umbrella conversion + the hollow-charts post-mortem cross-ref.

B. **Service drift (4):**
- `internal/helmwatch/` package (per-component HelmRelease watch + SSE `phase: "component"` events) was undocumented. Added to `IMPLEMENTATION-STATUS.md` catalyst-api row, `SOVEREIGN-PROVISIONING.md` §3 progress paragraph, `RUNBOOK-PROVISIONING.md` §3, `DEMO-RUNBOOK.md` Step 3, `ORCHESTRATOR-STATE.md` DoD step 5.
- `internal/store/` package + RWO PVC `catalyst-api-deployments` 1 Gi (Recreate, `fsGroup: 65534`) for deployment-state persistence (`/var/lib/catalyst/deployments/<id>.json` with secrets redacted) was undocumented. Added to the same five docs.
- New endpoints `GET /api/v1/deployments/<id>/kubeconfig` and `GET /api/v1/deployments/<id>/events` were undocumented. Added to IMPLEMENTATION-STATUS, SOVEREIGN-PROVISIONING, RUNBOOK-PROVISIONING, DEMO-RUNBOOK, ORCHESTRATOR-STATE.
- New env vars `CATALYST_PHASE1_WATCH_TIMEOUT` (default 60m) and `CATALYST_DEPLOYMENTS_DIR` (default `/var/lib/catalyst/deployments`) were undocumented. Added to IMPLEMENTATION-STATUS catalyst-api row + SOVEREIGN-PROVISIONING §3 footer.

C. **Architectural drift (7):**
- DAG provision view replaced by Admin app-card grid + per-app tabs (commits `4047ba1d`/`a75414a7`). `IMPLEMENTATION-STATUS.md` catalyst-ui row, `SOVEREIGN-PROVISIONING.md` §3 progress paragraph, `RUNBOOK-PROVISIONING.md` §3, `DEMO-RUNBOOK.md` Step 3, and `ORCHESTRATOR-STATE.md` DoD step 5 all updated to describe the AdminPage at `/sovereign/provision/$deploymentId` + the per-Application page at `.../app/$componentId` with Overview / Logs / Dependencies / Status tabs. The legacy `ProvisionPage.tsx` is now a 22-line re-export shim — that is called out so future contributors reading the old name don't get confused.
- All 11 bp-* charts converted to Helm umbrella shape at v1.1.0 (commits `43aff202`/`e42799fa`). The `12-component bootstrap kit` wording in five docs (README L79, IMPLEMENTATION-STATUS §7 bootstrap-kit row, SOVEREIGN-PROVISIONING §3 step 4, RUNBOOK §"What this runbook gets you", ORCHESTRATOR-STATE §"Architectural compliance") was reframed as "11 bp-* umbrella charts" — bp-powerdns is no longer in the franchised Sovereign bootstrap-kit (it's the authoritative DNS for *every other* Sovereign and ships only on Catalyst-Zero). The umbrella shape itself + the v1.0.0 / v1.0.1 hollow-charts post-mortem are cross-referenced to `BLUEPRINT-AUTHORING.md` §11.1 (already correct in BLUEPRINT-AUTHORING; this pass propagates the cross-ref).
- catalyst-api per-component HelmRelease watch (Phase 1) — the `internal/helmwatch/` package emits SSE events shaped `phase: "component", component: <id>, state: pending|installing|installed|failed|degraded`. The DEMO-RUNBOOK Step 3 phase table reframed: Phase 1 rows now show `component: <id>` shape instead of the legacy `<component>` shape. The "11 phases" framing was wrong for the new model — Phase 0 has 5 phases + Phase 1 emits one event per Application, of which the bootstrap-kit alone has 11; user-selected Apps add more.
- Cilium installed BEFORE Flux in cloud-init (`e571ec7a`/`54872009`) with `k8sServiceHost=127.0.0.1` to break the CNI bootstrap deadlock. The SOVEREIGN-PROVISIONING §3 step 3 row was rewritten ("k3s + Cilium + Flux bootstrap"), and the RUNBOOK troubleshooting matrix row "If `flux-bootstrap` shows `connection refused`" was retuned to point at the cloud-init Cilium Helm install (no longer at "Cilium CNI not yet up").
- Bootstrap-kit Flux split into `bootstrap-kit` (HelmReleases) + `infrastructure-config` (ProviderConfig + Crossplane Compositions, `dependsOn: bootstrap-kit`, `wait: true`) Kustomizations (`34c8de84`/`2da4c43c`). Added to SOVEREIGN-PROVISIONING §3 step 3 + step 5 + new RUNBOOK troubleshooting row for `infrastructure-config` stuck at `Ready=Unknown` + ORCHESTRATOR-STATE DoD step 5 + IMPLEMENTATION-STATUS Hetzner OpenTofu modules row.
- `null_resource.dns_pool` removed from `infra/hetzner/main.tf` (`330211d2`) — PDM is the single owner of pool-domain Dynadot writes. Updated SOVEREIGN-PROVISIONING §3 step 2 (no DNS in this module — PDM /v1/commit runs after tofu-output) + IMPLEMENTATION-STATUS Hetzner OpenTofu modules row (catalyst-dns helper / null_resource removed). Pass 1 had partly framed PDM as the canonical writer; this pass closes the gap by removing the residual "OpenTofu writes DNS" claims.
- OpenTofu CLI v1.11.6 + `infra/hetzner/` module bundled into the catalyst-api image (`9b6c297d`/`61c61226`) — the catalyst-api Pod IS the OpenTofu runner; no host-side install required. Updated SOVEREIGN-PROVISIONING §3 step 1, IMPLEMENTATION-STATUS catalyst-provisioner row + Hetzner OpenTofu modules row, ORCHESTRATOR-STATE DoD step 5, RUNBOOK §3 SSE phase 0 description.

D. **Process drift (3):**
- `RUNBOOK-PROVISIONING.md` §"Cost estimate for a default single-region run" still listed CPX21 / CPX31 SKUs from the pre-#176 era. Reality: tofu module default `control_plane_size = cx42` (16 GB Intel ~€25/mo) or wizard's recommended `cpx32` (8 GB AMD ~€17/mo); default `worker_size = cx32` with `worker_count = 0` for solo. Rewrote to range €25–€55/mo (or €80–€120/mo HA), called out empty `worker_size` for solo Sovereigns.
- `DEMO-RUNBOOK.md` Step 3 "You will see 11 phases in dependency order" — this framing is now wrong. The new SSE shape is 5 Phase-0 events (`tofu-init`, `tofu-plan`, `tofu-apply`, `tofu-output`, `flux-bootstrap`) plus per-component Phase-1 events (one per Application installed on the Sovereign — 11 minimum for bootstrap-kit, more once user-selected Apps are added). Reframed Step 3 prose to describe Phase 0 (banner) + Phase 1 (Application card grid) and updated the phase table to use `component: <id>` shape for Phase 1 rows.
- `RUNBOOK-PROVISIONING.md` §3 SSE event-stream prose previously said "Flux + Crossplane in the new cluster — Flux will now reconcile clusters/<sovereign-fqdn>/ from the public OpenOva monorepo, installing the 12-component bootstrap kit and bp-catalyst-platform umbrella in dependency order" (single Kustomization, 12-component framing). Reality: cloud-init now applies `bootstrap-kit` + `infrastructure-config` (two Kustomizations, `infrastructure-config dependsOn bootstrap-kit + wait: true`); Cilium is already installed by cloud-init Helm BEFORE Flux runs; the franchised bootstrap-kit is 11 bp-* (bp-powerdns is Catalyst-Zero only). Rewrote the prose accordingly.

E. **Cross-doc inconsistency (2):**
- `ORCHESTRATOR-STATE.md` L4 commit anchor `6afdb303` was Pass 2's anchor; updated to `934e519b` with a one-line summary of the post-Pass-2 architectural batch (admin landing surface, umbrella charts, helmwatch, deployments PVC, OpenTofu bundling, Cilium-before-Flux, bootstrap-kit / infrastructure-config split, ghcr-pull HelmRepository correction, kube-system Namespace removal, null_resource.dns_pool removal, CPX SKU family + empty worker_size, Containerfile go bump, blueprint-release CI hollow-chart guards).
- `PROVISIONING-PLAN.md` Group F status said "All 12 G2 wrapper charts (original 11 + bp-powerdns #167) + blueprint-release CI live" — same lag. Updated to "All 11 bp-* umbrella charts (the 10 leaves + bp-catalyst-platform; bp-powerdns ships separately for Catalyst-Zero only) at v1.1.0 — each declares its upstream chart under `Chart.yaml`'s `dependencies:` … the v1.0.0 / v1.0.1 artefacts were hollow … blueprint-release CI now runs four hollow-chart guards on every publish". Phase 5 prose rewritten to describe the umbrella authoring rule + the four guards (cross-ref to `BLUEPRINT-AUTHORING.md` §11.1).

F. **Stale terminology / banned terms (0):**
- The 11-grep banned-terms scan over `docs/`, `platform/*/README.md`, `products/*/README.md`, `core/README.md`, `README.md`, `CLAUDE.md` returned no NEW residuals beyond Pass 1/2's already-permitted exemptions (K8s Operator pattern, "multi-tenant" PSD2, AUDIT-PROCEDURE self-reference, historical-rename evidence inside this log). No new "MVP" / "for now" / "stub" / "iterative" / "demo" residuals introduced by the post-Pass-2 batch.

**Files updated (8, plus this VALIDATION-LOG entry):**

- `README.md` — repo-structure note: 11 bp-* umbrella charts at v1.1.0 (vs the prior "12-component bootstrap kit" framing); cross-ref to `BLUEPRINT-AUTHORING.md` §11.1; bp-powerdns separately on Catalyst-Zero only; v1.0.0 / v1.0.1 hollow-charts post-mortem note.
- `docs/IMPLEMENTATION-STATUS.md` — date-stamp Pass-3 marker; catalyst-ui row rewritten with the AdminPage / ApplicationPage / DAG-retired note; catalyst-api row rewritten with `internal/helmwatch/`, `internal/store/`, kubeconfig + events endpoints, env vars, deployments PVC + bundled OpenTofu CLI; PowerDNS row 1.0.6 → 1.1.0; catalyst-provisioner row rewritten for bundled Tofu CLI; Hetzner OpenTofu modules row rewritten for null_resource.dns_pool removal + Cilium-before-Flux + bootstrap-kit/infrastructure-config split + CPX-family-allowed regex + empty worker_size; bootstrap-kit row rewritten for 11 bp-* umbrella v1.1.0 + the four hollow-chart guards + ghcr-pull HelmRepository correction + kube-system Namespace removal.
- `docs/SOVEREIGN-PROVISIONING.md` — Pass-3 marker; §3 step 1 rewritten for bundled Tofu CLI + module; §3 step 2 rewritten for null_resource.dns_pool removal + CPX SKU family; §3 step 3 reframed as "k3s + Cilium + Flux bootstrap" with the cloud-init ordering + the bootstrap-kit / infrastructure-config split; §3 step 4 rewritten for 11 umbrella charts at v1.1.0 + ghcr-pull HelmRepository + kube-system Namespace removal + bp-powerdns scoped to Catalyst-Zero; §3 step 5 footnote at infrastructure/ path; §3 progress paragraph rewritten for AdminPage + helmwatch SSE shape; new "Deployment state persistence" paragraph for `internal/store/` + PVC + redacted secrets + `/api/v1/deployments/<id>/{kubeconfig,events}` + env vars; "Implementation status" footer updated for v1.1.0 + bundled Tofu CLI + module.
- `docs/RUNBOOK-PROVISIONING.md` — Pass-3 marker; §"What this runbook gets you" 12-component → 11 bp-* umbrella v1.1.0; §"Cost estimate" CPX21/CPX31 → CPX32 / cx42 (or empty for solo); §3 rewritten as "Watch the Sovereign Admin landing surface" with the AdminPage / per-Application page / Phase-0 / Phase-1 SSE shape / persisted-events replay; troubleshooting matrix `flux-bootstrap connection refused` row retuned for cloud-init Cilium Helm install; `bp-cilium Ready=Unknown` row reframed for cloud-init Helm install failures; new troubleshooting row for `infrastructure-config Ready=Unknown` while bootstrap-kit is Ready.
- `docs/DEMO-RUNBOOK.md` — Step 3 prose rewritten for the AdminPage / ApplicationPage / Phase-0-as-banner + Phase-1-as-card-grid model; phase table reframed with `component: <id>` shape on Phase-1 rows; the "11 phases" framing is gone; cross-refs to bundled Tofu + cloud-init Cilium-before-Flux + bootstrap-kit / infrastructure-config split + helmwatch SSE shape + kubeconfig endpoint.
- `docs/PLATFORM-POWERDNS.md` — `bp-powerdns:1.0.6` → `1.1.0` in 3 places (helmrelease pull comment, acceptance checklist, runbook commit message).
- `docs/PROVISIONING-PLAN.md` — Group F row updated to "All 11 bp-* umbrella charts at v1.1.0 + four hollow-chart guards" with `43aff202`/`e42799fa`/`54418bd5`/`35dcb84d`/`bdeb0f54` added to the commit list; Phase 5 prose rewritten to describe the umbrella authoring rule + the four guards (cross-ref to `BLUEPRINT-AUTHORING.md` §11.1).
- `docs/ORCHESTRATOR-STATE.md` — `Latest commit on main` advanced from `6afdb303` to `934e519b` with a long post-Pass-2 batch summary; "Blueprints" architectural-compliance bullet reframed for 11 umbrella v1.1.0 leaves + bp-powerdns Catalyst-Zero-only + hollow-charts post-mortem cross-ref + the four CI guards; DoD step 5 rewritten for AdminPage redirect + helmwatch SSE shape + bundled Tofu + cloud-init Cilium-before-Flux + bootstrap-kit / infrastructure-config split + deployments-PVC persistence + new endpoints.

**Reconcile Pass 3: drift fixed across 8 files (+ this VALIDATION-LOG entry; 1 commit, branched `docs/reconcile-pass-3` from main @ `934e519b`).**

**Escalations:** none. Every drift item identified was a doc-side claim that fell behind the post-Pass-2 architectural batch (admin landing surface, umbrella conversion, helmwatch, deployments PVC, OpenTofu bundling, Cilium-before-Flux, bootstrap-kit / infrastructure-config split, etc.); bringing the docs into line did not require any code change. The `12-component bootstrap kit` anchor from Pass 1/2 was reconciled to "11 bp-* umbrella charts" — bp-powerdns deploys on Catalyst-Zero only, so the franchised Sovereign install set is 11 (10 leaves + bp-catalyst-platform umbrella). No new lessons.

**Lessons:** none new. The drift surface was the expected post-architectural-batch shape (admin landing surface replaces DAG, umbrella conversion + hollow-charts post-mortem, per-component HelmRelease watch + persistence, bundled OpenTofu CLI, Cilium-before-Flux cloud-init ordering, bootstrap-kit / infrastructure-config Kustomization split, OCI HelmRepository url + ghcr-pull secretRef correction, null_resource.dns_pool removal). The reconcile-skill's six-category rubric continues to bound the patch set cleanly.

---

### Reconcile Pass 2 — 2026-04-29 (drift fixed)

**Date:** 2026-04-29. **Branch:** `docs/reconcile-pass-2`. **Base:** main @ `6afdb303` (post-#162/#173/#175/#176/#179 batch — pixel-match component cards, family chips, dep audit, dynamic provision DAG, worker SKU + count selector, wizard step reorder, canonical upstream logos, smoke-test fix, TOFU_WORKDIR pin to /tmp, marketplace-pages design-token alignment). **Skill:** `reconcile-catalyst-docs full`. **Trigger:** post-major-architectural-changes batch. **Verdict:** ✅ drift fixed — canonical docs realigned with the wizard's new step ordering, the per-provider SKU model, the flat single-grid StepComponents UX with family chips, the canonical-upstream logo set (44 SVG + 14 PNG), the corrected product-family dependency graph (no family-level deps), and the writable `/tmp/catalyst/tofu` workdir.

**Ground truth at HEAD `6afdb303`:**
- 56 platform components (`ls platform/ | wc -l = 56`) — anchor unchanged from Pass 1.
- 6 products (axon, catalyst, cortex, fabric, fingate, relay).
- 11 platform `blueprint.yaml` files + 14 Helm `Chart.yaml` files (the 12-component bootstrap kit minus opentofu/sealed-secrets which carry blueprint metadata in chart values, plus 3 trees with their own Chart.yaml: products/axon, products/catalyst, plus external-dns leaf chart).
...
- Live in `openova-system` on Catalyst-Zero: `pool-domain-manager`, `powerdns` (`bp-powerdns:1.0.6`), `dnsdist`, CNPG-managed `pdm-pg` + `pdns-pg`. Live in `catalyst`: catalyst-api (now with `CATALYST_TOFU_WORKDIR=/tmp/catalyst/tofu` env, 1 Gi memory limit, 2 Gi `/tmp` emptyDir per `27527e4c`) + catalyst-ui.
- componentGroups.ts: 62 unique component IDs across 11 groups (5 core + 2 side + 4 a-la-carte) — unchanged from Pass 1's 63-with-duplicates count, now corrected to distinct-id count. CORTEX `familyDependencies = []` after `0b6bb3ea` audit; LibreChat → ferretdb (was cnpg); Grafana → [] (dropped seaweedfs).
- 58 logos vendored under `public/component-logos/` (44 SVG + 14 PNG) — replaced the prior 62 stylised in-house marks at `169b1d1c`/`30ff318d` with the canonical upstream set (CNCF artwork repo + project repos, OpenOva-curated for axon/bge/continuum/specter; PowerDNS keeps `logoUrl: null` letter-mark fallback).
- Wizard step ordering canonical from `STEPS` in `src/pages/wizard/WizardPage.tsx` — the in-flight fix-branch reorders the on-disk Step 2/3 from "Provider→Credentials→Topology" to "Topology→Provider→Credentials" so the operator-facing canonical is **Org → Topology → Provider → Credentials → Components → Domain → Review**. Doc patches in this pass reflect the post-fix shape; the on-disk `STEPS` array will catch up when the parallel branch lands.
- StepComponents (post `d3346441` + `b0ec0c43`): single flat marketplace card grid with family chips on each card + search + product-family chip filter; two tabs ("Choose Your Stack" + "Always Included"); per-card click affordances split — family chip routes to family portfolio, card body routes to product detail, only Select / Selected button toggles store.
- `provision.html` (post `a02f33ce` / `60e403ae`): dynamic DAG built at runtime from `window.CATALYST_CATALOG` + `localStorage('openova-catalyst-wizard')` + the catalyst-api SSE stream — no hardcoded component lists.
- catalyst-build smoke test (post `6a7d2dd8`): asserts every PNG/SVG path the catalog references resolves to HTTP 200 — a missing or mis-cased asset fails the build.

**Drift identified and fixed (six categories per skill spec):**

A. **Numerical drift (1):**
- `README.md` L78 claimed "every folder under `platform/` and `products/` (except `products/axon/`) contains only a `README.md`" — stale since Pass 105 (the 11 G2 charts + Pass 1's bp-powerdns landed) and Pass 1 (the catalyst chart). Reality: 11 platform/ folders carry full `chart/` + `blueprint.yaml`, plus `products/axon/` and `products/catalyst/`. Updated to call out the 12-component bootstrap kit as built and the remaining 45 platform / 4 product folders as design-stage. Cross-checked against `IMPLEMENTATION-STATUS.md §7` (already correct).

B. **Service drift (0):** Pass 1 already covered PDM, registrar adapters, bp-powerdns, dnsdist; no new services landed since.

C. **Architectural drift (3):**
- `RUNBOOK-PROVISIONING.md` "What can go wrong" §, `SOVEREIGN-PROVISIONING.md` §3 footer, `DEMO-RUNBOOK.md` decommission code-block all referenced `/var/lib/catalyst/tofu/<sovereign-fqdn>/` as the OpenTofu workdir. Reality (commit `27527e4c`, `products/catalyst/chart/templates/api-deployment.yaml` L72-73): `CATALYST_TOFU_WORKDIR=/tmp/catalyst/tofu`, backed by a 2 Gi `/tmp` emptyDir because UID 65534 cannot write to `/var/lib/...`. Updated all three with the env-var name, the commit ref, and the rationale; updated DEMO-RUNBOOK's manual-fallback `kubectl exec` command to `cd /tmp/catalyst/tofu/...` and corrected the namespace from `catalyst-system` to `catalyst`.
- `IMPLEMENTATION-STATUS.md` L49 claimed the wizard has 7 steps in order "Org → Provider → Credentials → Infrastructure → Topology → Components → Review". Reality: Domain is its own step now (#169) and there is no separate Infrastructure step (Topology absorbs it). Updated to canonical order **Org → Topology → Provider → Credentials → Components → Domain → Review** with cross-refs to #176 (PROVIDER_NODE_SIZES) and #d3346441/#b0ec0c43 (single-grid + family chips + product/family routes).
- `PRODUCT-FAMILIES.md` L38-39 + L90 + the Specter / BGE cascade examples all asserted CORTEX has `familyDependencies: ['fabric']`. Audit at `0b6bb3ea` flipped this to `[]` because the operator's true cross-family needs are localised at component level (`langfuse → cnpg`, `librechat → ferretdb → cnpg`) and dragging the entire FABRIC family in was over-broad (operator: "selecting Spector brings the entire fabric family — there is no such dependency in reality"). Rewrote §"Product graph", §"Product registry" CORTEX row, and the Specter / BGE cascade narratives accordingly.

D. **Process drift (3):**
- `RUNBOOK-PROVISIONING.md` §2 wizard table listed Pass 1's order (Org → Domain → Topology → Provider → Credentials → Components → Review) which is now stale — the in-flight wizard step-reorder branch puts Topology before Provider/Credentials so the operator picks workload sizing first, and Domain absorbs the sovereign-admin email so it pairs with the Sovereign's external surface. Rewrote the 7-row table with the new ordering, called out per-step concerns: Step 1 (Organisation: profile fields, no email here), Step 2 (Topology: SKU pickers via `PROVIDER_NODE_SIZES[provider]` + worker count + HA toggle), Step 5 (Components: single flat grid with family chips per `b0ec0c43`/`d3346441`).
- `DEMO-RUNBOOK.md` Step 2 wizard table mirrored the same stale ordering plus stale `cpx21`/`cpx31` SKU defaults (the worker SKU + count selector at `b96a03a5`/`8aec6244` introduced `cx32`/`cx42` from the canonical Hetzner catalog in `PROVIDER_NODE_SIZES.hetzner`). Rewrote with the new 7-step canonical order and `cx32`/`cx42` defaults, moved the sovereign-admin email capture to Step 6 (Domain), updated the Components rows to describe the single flat grid + Choose-Your-Stack / Always-Included tabs.
- `RUNBOOK-PROVISIONING.md` "Before you start" pre-flight table renamed "Organisation name + email" to "Organisation profile" (no email captured at Step 1 anymore) and added the SKU-via-`PROVIDER_NODE_SIZES[provider]` callout to the Topology choice row. The "Sovereign domain" row gained a sentence noting the Step 6 capture position (paired with the deployed footprint).

E. **Cross-doc inconsistency (3):**
- `ORCHESTRATOR-STATE.md` L4 "Latest commit on main: `dd578d1c`" was Pass 1's anchor; updated to `0b432dd7` with a one-line summary of the post-Pass-1 batch (#162/#175/#176 + dep audit + dynamic DAG + step reorder + canonical logos + smoke-test + TOFU_WORKDIR pin). The Group / waterfall snapshot below is unchanged structurally.
- `.github/workflows/blueprint-release.yaml` L12 comment said "all 11 bootstrap-kit charts" — Pass 1 raised the canonical bootstrap-kit to 12 (added bp-powerdns at #167) but the CI comment lagged. Updated to "all 12 bootstrap-kit charts (the original 11 plus bp-powerdns added at #167)". The path-matrix CI itself already publishes bp-powerdns (verified at `bp-powerdns:1.0.6` deployed in `openova-system`), so this is a pure comment realignment, not a build-graph change.
- `PROVISIONING-PLAN.md` L17 Group F status said "All 11 G2 wrapper charts + blueprint-release CI live" — same lag as the workflow comment; updated to "All 12 G2 wrapper charts (original 11 + bp-powerdns #167) + blueprint-release CI live" with `0190c605` added to the commit list.

F. **Stale terminology / banned terms (1):**
- `COMPONENT-LOGOS.md` opening prose described the 62 SVG marks as "stylised brand-color marks authored in-house, not copies of the upstream projects' official logo files" — true at Pass 1 but false after `169b1d1c`/`30ff318d` swapped in the canonical upstream set (44 SVG from CNCF artwork + project repos, plus 14 PNG for projects whose canonical asset is published as PNG: coraza, external-dns, ferretdb, langfuse, loki, mimir, netbird, ntfy, openmeter, strongswan, syft-grype, tempo, trivy, vllm). Rewrote the opening + "Current asset status" + "License note" sections to source-attribute the new asset set, document the SVG/PNG-mixed convention, name PowerDNS as the explicit `logoUrl: null` exception, and cross-reference the CI logo smoke-test at `6a7d2dd8`. Also updated the StepComponents description ("3-column grid that pixel-mirrors the SME marketplace" → "single flat marketplace card grid (no per-family section headers) with a clickable family chip on each card and a search + product-family chip filter at the top — the layout that won out after the operator's 'fragmented page' feedback at #b0ec0c43"). The 11-grep banned-terms scan over `docs/`, `platform/*/README.md`, `products/*/README.md`, `core/README.md`, `README.md`, `CLAUDE.md` returned no NEW residuals beyond Pass 1's already-permitted exemptions (K8s Operator pattern, "multi-tenant" PSD2, AUDIT-PROCEDURE self-reference, historical-rename evidence inside this log).
...
- `.github/workflows/blueprint-release.yaml` — header comment updated to "all 12 bootstrap-kit charts (the original 11 plus bp-powerdns added at #167)".

**Reconcile Pass 2: drift fixed across 10 files (+ this VALIDATION-LOG entry; 1 commit, branched `docs/reconcile-pass-2` from main @ `6afdb303`).**

**Escalations:** none. Every drift item was a doc-side claim that fell behind the post-Pass-1 batch of merged + in-flight wizard work; bringing the docs into line did not require any code change. The wizard step-reorder is in-flight on a parallel branch — these doc patches are intentionally landed first so the in-flight code merge will arrive into a doc set that already names the canonical order, avoiding a second drift round.

**Lessons:** none new. The drift surface was the expected post-architectural-batch shape (wizard step reorder, per-provider-SKU rewrite, flat-grid StepComponents, dependency-graph audit, canonical-upstream logos, runtime workdir override). The reconcile-skill's six-category rubric continues to bound the patch set cleanly.
