...
| 18 | `bp-harbor` | Container registry — avoids Docker Hub rate limits | 🟡 in flight — [#383](https://github.com/openova-io/openova/issues/383) chart rework on post-#425 vendor-agnostic shape (`flux-system/object-storage` Secret, `.Values.objectStorage.s3.*`) — agent dispatched |
...
| #383 | 🟡 in flight — bp-harbor chart rework on post-#425 vendor-agnostic shape: `templates/objectstorage-credentials.yaml` mirroring velero, `.Values.objectStorage.s3.*` block, `valuesFrom: flux-system/object-storage` in per-Sovereign overlays, `dependsOn: bp-seaweedfs` REMOVED (SeaweedFS not in minimal Sovereign set per ADR-0001 §13). Agent dispatched on worktree `.worktrees/omantel-383`, branch `fix/383-omantel`. | (PR pending) | gates Phase 6 (#385 catalyst-platform single-blueprint verify) |
