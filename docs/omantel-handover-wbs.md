...
flowchart TB
    ...
    class T316,T327,T338,T370,T371,T373,T375,T376,T377,T378,T379,T380,T381,T382,T387,T392 done
    class T331,T374,T383,T384 wip
...
| #331 | 🟡 in flight — bp-external-secrets split into bp-external-secrets@1.1.0 (controller) + bp-external-secrets-stores@1.0.0 (CR consumers, dependsOn controller) following PR #247 pattern. Resolves CRD-ordering install failure on otech (worktree `.worktrees/omantel-331`, branch `fix/331-omantel`) | (PR pending) | |
...
| #374 | 🟡 in flight — wizard-driven NS delegation `omani.works` parent zone → `omantel.omani.works` onto omantel's PowerDNS via Dynadot API; integrate with #373 powerdns-webhook for cert-issuance-after-delegation (worktree `.worktrees/omantel-374`, branch `fix/374-omantel`) | (PR pending) | |
...
| #383 | 🟡 in flight — bp-harbor chart rework (unhardcode SeaweedFS endpoint at `platform/harbor/chart/values.yaml:100-109`; consume `flux-system/hetzner-object-storage` Sealed Secret from #371; update `components.ts` deps to drop seaweedfs) (worktree `.worktrees/omantel-383`, branch `fix/383-omantel`) | (PR pending) | |
| #384 | 🟡 in flight — bp-velero Hetzner Object Storage wiring (BackupStorageLocation S3 endpoint via `existingSecret: hetzner-object-storage` from #371) (worktree `.worktrees/omantel-384`, branch `fix/384-omantel`) | (PR pending) | |
| #385 | (parked) | | |
