...
| **Not-a-defect — reclassify** | **34** (KC admin console → login form) | **INTENDED per #3934 (CLOSED)**: `security-admin-console` bound to the non-delegating `browser-no-idp` flow (`configmap-sovereign-realm.yaml:574-578,1035-1046`; test guard `realm-flow-subflow-resolution.sh:179-196`) to avoid the force-bounce `cookie_not_found` lockout. Silent-SSO is for app/operator surfaces, NOT the KC-native admin console. The row's assertion is mis-specified. | recommend founder reclassify; correct pass = the "Catalyst PIN Sign-In" button completes (broker restored by #5278) — verify on hw279 |
