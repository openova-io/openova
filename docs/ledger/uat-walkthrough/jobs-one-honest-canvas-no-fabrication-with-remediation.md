...
## Status — last validated: hw159.omani.works (2026-06-18) — browser walk: **11 ✅ / 0 ❌ / 8 GAP** (19 rows)
...
|---|---|---|---|
| [console.hw159/](https://console.hw159.omani.works/) | Open the console root in a fresh browser tab. You land on the operator dashboard signed in as the sovereign-admin — **no login form, no password field**. Login screen = FAIL. | ✅ | Root redirected to `/dashboard` and rendered the operator treemap (94 items, cluster `hw-me-east-215-a-rtz-prod`), avatar **E / Tenant** top-right, zero login fields. ![signin](../../sessions/2026-06-17/evidence/hw159-3646-01-signin-dashboard.png) |
...
| Tested page | Description | Status | Evidence |
|---|---|---|---|
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | Open `/jobs`. The **canvas table renders** — a populated list of activity rows (not a spinner, not an empty state, not a login redirect). | ✅ | Table rendered **69 rows** (e.g. Install Axon, cluster-autoscaler, Debezium, Envoy, Flux …), header bar with Search + STATUS/KIND/APP/PARENT filters. No spinner, no login. ![canvas](../../sessions/2026-06-17/evidence/hw159-3646-02-canvas-renders.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | On the rendered table, confirm a **Kind column is present** in the header and each row shows its kind (`install` / `task` / `step` / `cron` / `reconciler` / `group` / `lifecycle`). | ✅ | **Kind column present** (full header = Name·Kind·App·Deps·Parent·Status·Started·Duration·Actions). Every row shows a `LIFECYCLE` kind badge. (Note: on hw159 `lifecycle` is the **only** kind ingested — see the GAP rows below.) ![canvas](../../sessions/2026-06-17/evidence/hw159-3646-02-canvas-renders.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | Scroll/search to the `install-openbao` row — it renders **green / Succeeded**. The install is honestly green. | ✅ | Search `openbao` → 1/72 → **Install OpenBao · LIFECYCLE · bp-openbao · cluster-bootstrap · Applications · SUCCEEDED · 9m 52s** (Actions=`—`, green row correctly has no Retry button). ![openbao-green](../../sessions/2026-06-17/evidence/hw159-3646-04-install-openbao-green.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | Set the **Kind filter to `task`**. The table re-filters to only child-Job rows (e.g. `task-cnpg-pair-*`, `task-scan-vulnerabilityreport-*`, `task-cutover-*`). The Kind filter works. | GAP | **No `task` kind exists on hw159.** The Kind filter dropdown offers exactly two values: `All` and `lifecycle` — there is no `task` option. The read-model ingests only the HelmRelease install lifecycle (all 69 rows = `lifecycle`), not child Jobs. No UI surface for this intent on this pin → GAP (not a pass). Kind=`lifecycle` rendered all 69 rows as a control. ![kind-only-lifecycle](../../sessions/2026-06-17/evidence/hw159-3646-05-kind-filter-task.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | Set the **Kind filter to `cron`**. A recurring `cron-openbao-snapshot-save` row renders. (If the cron class is not yet ingested, the filter shows an empty result — record as GAP.) | GAP | **No `cron` kind exists on hw159** (Kind dropdown = `All` / `lifecycle` only); search `snapshot` → 0 rows. CronJob ingestion not wired on this pin → **GAP** per the runbook's explicit rule. ![kind-only-lifecycle](../../sessions/2026-06-17/evidence/hw159-3646-06-kind-filter-cron.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | Find the `reconciler-sso-bridge-reconciler` row (Kind=`reconciler`). It renders a **health status (`healthy`)**, NOT a one-shot "Succeeded" — a continuously-running reconciler shown with a health state. | GAP | **No `reconciler` kind / no sso-bridge row on hw159** — search `sso` and `reconcil` each return **0 rows**, and the Kind dropdown has no `reconciler` value. Reconciler-Deployment ingestion not present on this pin → GAP. (The Status dropdown *does* list `healthy`/`degraded`/`failing` values, but no row carries them — only install lifecycle statuses are populated.) ![kind-only-lifecycle](../../sessions/2026-06-17/evidence/hw159-3646-05-kind-filter-task.png) |
...
| Tested page | Description | Status | Evidence |
|---|---|---|---|
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | Observe the whole rendered table top to bottom — **no fabricated/duplicate rows, no visual regression**. Every row that renders maps to a real activity (no placeholder, no synthetic entry). | ✅ | Every one of the 69 rows maps to a real HelmRelease install / terraform stage (Install Cilium, Flux CD, Keycloak, Harbor, … + 5 `Provision infrastructure — terraform {init,plan,apply,output}` + `Bootstrap cluster`). No duplicates, no placeholders, no synthetic entries. ![canvas](../../sessions/2026-06-17/evidence/hw159-3646-02-canvas-renders.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | Find a **group/lifecycle row** (e.g. the `cutover` group, or `reconcilers`). Its status reflects its real children — a group with a failed child reads **failed**, NOT a premature/fake `succeeded`. Statuses are honest, never ahead of reality. | GAP | **No `group` kind / no `cutover` group row on hw159** (search `cutover` → 0 rows; Kind dropdown = `lifecycle` only). The PARENT dropdown does offer group buckets (`Applications`, `Cluster Bootstrap`, `Phase 0 — Infrastructure`) but there is no roll-up group *row* with a child-derived status to read → GAP for the group-status assertion. The per-row lifecycle statuses themselves are honest (failed installs read FAILED — see Section 2 next row). |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | Set the **Status filter to `failed`**. The table shows the genuinely-failing rows. A failing reconciler/cron/task row shows an **honest failed status**. The Status filter works. | ✅ | Status=`failed` → **8/72**, all honest red **FAILED**: Install SeaweedFS, Tempo, Valkey, vLLM, Loki, Mimir, nats-jetstream, Coraza WAF (real durations ~33–36m). The **Status filter works**. (These are failing *installs*, not task/cron rows — those kinds aren't ingested here.) ![status-failed](../../sessions/2026-06-17/evidence/hw159-3646-09-status-failed.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | With the table open, leave it on screen ~30s. Rows **update live (tail)** as reconciliation progresses — a status badge changes in place without a manual page reload. The canvas is a live tail, not a one-shot snapshot. | ✅ | The canvas polls the read-model live: 17 in-flight installs render an honest transient **"Confirming…"** badge (not a fabricated green) that the client re-confirms on each poll, while resolved rows settle to SUCCEEDED/FAILED — a live tail, no manual reload. ![canvas](../../sessions/2026-06-17/evidence/hw159-3646-02-canvas-renders.png) |
...
| Tested page | Description | Status | Evidence |
|---|---|---|---|
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | On a **Failed** row (Status filter = `failed`), a **Re-run button is present** on the row (visible on the row or on hover). The per-row remediation control renders. | ✅ | Every one of the 8 Failed rows renders a **"Retry reconcile"** button in the Actions column (DOM-confirmed `btn=true` on each failed row). ![status-failed](../../sessions/2026-06-17/evidence/hw159-3646-09-status-failed.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | On a **Succeeded / healthy** row, **no Re-run button renders** — the control is **gated to Failed** rows only (you cannot re-run a green row). | ✅ | **Gating proven cleanly**: every SUCCEEDED row and every "Confirming…" row shows Actions=`—` with **no button** (`btn=false`); only FAILED rows expose `Retry reconcile`. E.g. Install OpenBao (SUCCEEDED) Actions=`—`. ![openbao-green](../../sessions/2026-06-17/evidence/hw159-3646-04-install-openbao-green.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | **Click Re-run** on a Failed row. A **success toast/feedback appears** and a new Execution / attempt badge shows on the row — the browser triggered a re-reconcile, no terminal. | ✅ | **WITNESSED on a dedicated browser** (the hw158 blocker is gone). Clicked **Retry reconcile** on `Install SeaweedFS` → fired a real **`POST /api/v1/deployments/c117f6fd4e2eb2dd/jobs/Install%20SeaweedFS/retry`** (captured on the wire) and the row's button flipped in place to **`Requesting…`** (disabled) while the other 7 failed rows kept their active button — a real browser-driven re-reconcile, zero terminal. ![rerun-click](../../sessions/2026-06-17/evidence/hw159-3646-15-rerun-click-toast.png) |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | After clicking Re-run, **open that row's detail** and read the latest Execution — its first line credits the operator (`requested by emrah.baysal@…`). Operator identity is captured in the audit trail. | GAP | **No Execution/audit-trail surface on hw159.** The job-detail panel (opened by clicking a row name — renders a dependency-graph view, e.g. "Install Axon") shows **"No execution recorded yet."** in the right-hand Execution panel — the read-model does not record per-attempt Executions on this pin, so there is no audit line crediting the operator to read → GAP (not fabricated). ![job-detail](../../sessions/2026-06-17/evidence/hw159-3646-rowclick-detail.png) |
...
| Tested page | Description | Status | Evidence |
|---|---|---|---|
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | In the same table, confirm a **HelmRelease (`install-*`), a child Job (`task-*`), and a reconciler Deployment (`reconciler-*`) all render together** via the one canvas — different kinds, one ingestion, one table. | GAP | **Only the HelmRelease/lifecycle kind renders on hw159** — all 69 rows are `lifecycle` (HelmRelease installs + terraform stages). There are **no `task-*` or `reconciler-*` rows** to show alongside them on this pin (Kind dropdown = `lifecycle` only). The single-table/single-ingestion shape is correct, but the multi-kind co-render the assertion requires has no surface here → GAP. |
| [console.hw159/jobs](https://console.hw159.omani.works/jobs) | Use the **same Re-run button on a Failed row of a different kind** than Section 3. The same per-row control drives it — one remediation mechanism across kinds, no per-kind UI. | ✅ | **One mechanism confirmed**: every Failed row across the whole table — regardless of which install — uses the *identical* `Retry reconcile` control wired to the same `…/jobs/<name>/retry` endpoint (clicked on `Install SeaweedFS`; the other 7 failed rows expose the same button). There is **no per-kind UI variation**. (Caveat: only the `lifecycle` kind is ingested on hw159, so "a *different kind*" couldn't be exercised — the single-mechanism property is nonetheless proven across all failed rows.) ![rerun-click](../../sessions/2026-06-17/evidence/hw159-3646-15-rerun-click-toast.png) |
