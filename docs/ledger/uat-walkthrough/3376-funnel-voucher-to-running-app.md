...
## Status — last validated: hw158 (2026-06-17) — POST-FIX RE-WALK after #3754 (org-controller per-Org GitRepository `secretRef`) + #3749 (git-auth secret hook) landed live (chart bp-catalyst-platform@1.4.664)

> **POST-FIX UPDATE (2026-06-17, #3754 + #3749 live on hw158):** The earlier hw158 walk found the terminal blocked by the per-Org `GitRepository catalyst-tenant-walk-stranger-co` being **READY:False — "authentication required"** (no repo-level secretRef). **That auth-strand is now FIXED and PROVEN LIVE:** the GitRepository carries `spec.secretRef.name=openova-sme-tenants-git-auth` (the org-controller deployment now sets `CATALYST_FLUX_GITEA_SECRET_REF=openova-sme-tenants-git-auth`, image `organization-controller:42c4213`), the referenced secret EXISTS in `flux-system` (hook-created), the GitRepository is **READY=True** (`stored artifact for revision 'main@sha1:12a8dee2…'`), the Kustomization `catalyst-tenant-walk-stranger-co-vcluster` is **READY=True**, and the Org namespace `walk-stranger-co` now **EXISTS** (Active). So **B14 advances** (namespace + per-Org GitRepo auth + Kustomization now real). **BUT the terminal (B15–B18) STILL FAILS** — the app does not run because the per-Org **vCluster HelmRelease `walk-stranger-co/vcluster` install FAILED on a NEW downstream blocker**: kyverno `harbor-proxy-pull` (`image-from-harbor-proxy-workload`) **denies** the `vcluster@0.33.3` chart's StatefulSet initContainer image (not re-tagged through the Sovereign's local Harbor proxy glob). Org CR condition is now `Ready=False(VClusterProvisioning)` (was the auth-strand). The per-Org realm + base-RBAC remain DISABLED (`PerOrgRealmProvisioned=False(RealmDisabled)`, `IacRepoBootstrapped=False(BootstrapDisabled)`). Net: the #3754 fix did exactly what it promised and unblocked the auth layer, surfacing the next blocker (vcluster image must pull through Harbor). B14 root-cause text corrected below; B15–B18 stay ❌ with the corrected (vcluster/kyverno) root cause.

- **Verdict: PARTIAL — funnel front-half PROVEN live, terminal (running app in own Org console) FAILED.** Honest tally across the 29 rows (A1–A6, B1–B20, C1–C3): **11 PASS / 11 FAIL / 5 VISUAL-PENDING / 2 PARTIAL**. The 11 PASS are the front-half (voucher preview, entropy gate, due-zero checkout, rate-limit, sovereign-clean bundle, real Org-CR mint); the 11 FAIL are the terminal half (no running app, no per-Org console/realm) + the cutover precondition rows this fresh prov never ran. **Post-#3754: the per-Org GitRepo auth strand inside B14/B17 is cleared (namespace now exists, GitRepository+Kustomization READY=True); the terminal failure has moved DOWNSTREAM to the vcluster-install kyverno block.**
...
- **What FAILED LIVE (the actual acceptance — purchased app RUNNING in the customer's own Org console):** the Org **still never reaches a running app** — but POST-#3754 the blocker has moved. `kubectl get ns walk-stranger-co` now shows **Active** (namespace EXISTS post-#3754, was absent); but `kubectl get pods -A \| grep -i wordpress` -> **NO WordPress pod cluster-wide**; **NO per-Org console HTTPRoute**; `https://console.walk-stranger-co.omani.homes` -> **connection refused (HTTP 000)**. **Post-#3754 the strand chain is:** the per-Org **`GitRepository catalyst-tenant-walk-stranger-co` is now READY=True** (auth FIXED — `secretRef=openova-sme-tenants-git-auth` set by the org-controller per #3754; referenced secret hook-created per #3749) and its **Kustomization READY=True** -> the vCluster HR is now applied, **but the vCluster HelmRelease `walk-stranger-co/vcluster` (chart `vcluster@0.33.3`) install FAILED**: kyverno `harbor-proxy-pull` / `image-from-harbor-proxy-workload` **denied** the StatefulSet (initContainer image not re-tagged through the Sovereign's local Harbor proxy glob). So no vCluster pod -> no per-Org console -> no WordPress. The **#3754 auth-strand is cleared**; the remaining terminal FAIL (B15–B18 + C1–C3) is the **vcluster-image/kyverno** blocker. (The earlier "`sme-tenants` ref-lock race" mis-marking the funnel `failed` at step 2 is orthogonal and still observed at the marketplace timeline level — B12.) |
...
| B14 | (operator window) `kubectl get ns \| grep <slug>` and `...logs deploy/organization-controller` | The Org namespace exists; org-controller logs show fan-out of per-Org **vCluster + Keycloak group/realm + Gitea org + base RBAC** | **PARTIAL — POST-#3754 the namespace + per-Org GitRepo auth now resolve; vCluster install then FAILS on a NEW (kyverno) blocker; realm/RBAC still disabled.** ✅ **Namespace now EXISTS** — `kubectl get ns walk-stranger-co` -> `Active` (was "does NOT exist" pre-#3754). ✅ **Per-Org GitRepository auth FIXED (#3754):** `gitrepository/catalyst-tenant-walk-stranger-co -n flux-system` now carries `spec.secretRef.name=openova-sme-tenants-git-auth` and is **READY=True** (`stored artifact for revision 'main@sha1:12a8dee2…'`) — the org-controller deploy now sets `CATALYST_FLUX_GITEA_SECRET_REF=openova-sme-tenants-git-auth` (image `organization-controller:42c4213`); the referenced secret exists in `flux-system` (#3749 hook). ✅ **Kustomization** `catalyst-tenant-walk-stranger-co-vcluster` **READY=True** (`Applied revision: main@sha1:12a8dee2…`). org-controller logs still show fan-out: `keycloak_group:e5c61e02-…, gitea_org:walk-stranger-co, vcluster:walk-stranger-co`. ❌ **BUT the vCluster HelmRelease `walk-stranger-co/vcluster` (chart `vcluster@0.33.3`) install FAILED** — kyverno `harbor-proxy-pull` / `image-from-harbor-proxy-workload` **denied** the StatefulSet because its initContainer image is not re-tagged through the Sovereign's local Harbor proxy glob (`*/proxy-*/*` or `*/openova-io/*`). So the vCluster pod never starts. CR conditions now: `Ready=False(VClusterProvisioning)`, `PerOrgRealmProvisioned=False(RealmDisabled — per-Org realm auto-wiring disabled in this controller deployment)`, `IacRepoBootstrapped=False(BootstrapDisabled)`. So Keycloak group + Gitea org fanned out and the namespace+GitRepo+Kustomization are now real, but **the vCluster can't install (Harbor-glob/kyverno)** and **per-Org realm + base RBAC are DISABLED in this controller build**. |

### B.4 — The customer lands signed-in in their OWN console with the app RUNNING (the terminal)

| # | Go to / do | You should see | Verdict (hw158) |
|---|---|---|---|
| B15 | After checkout the marketplace **redirects** the customer to their per-Org console | Browser URL becomes `https://console.<slug>.<pool-tld>` with publicly-trusted TLS | **FAIL — per-Org console host does NOT exist (downstream of the failed vCluster install).** `kubectl get httproute -A \| grep walk-stranger` -> **nothing** (no per-Org console route). `curl --max-time 12 https://console.walk-stranger-co.omani.homes` -> **HTTP 000, "Connection refused"**. Post-#3754 the namespace + GitRepo are now real (see B14), but the per-Org console lives **inside** the vCluster, and the **vCluster HelmRelease install FAILED** (kyverno harbor-proxy denies `vcluster@0.33.3` images) -> no vCluster -> no per-Org console host. Prior `05-post-launch-redirect.png` shows the redirect landing on a host with `ERR_CERT_COMMON_NAME_INVALID` (no cert, no vCluster behind it). |
...
| B17 | In the Org console, open the purchased app's card and click **Open** | The purchased app (e.g. WordPress) **serves** at its own FQDN — **TERMINAL SCREENSHOT = the running app** | **FAIL — THE ACCEPTANCE STILL FAILS — no running app — but the root cause MOVED downstream post-#3754.** `kubectl get pods -A \| grep -i wordpress` -> **NO WordPress pod cluster-wide**. **Corrected root cause (post-#3754):** the per-Org `GitRepository catalyst-tenant-walk-stranger-co` is now **READY=True** (auth strand FIXED — secretRef set, see B14) and the namespace `walk-stranger-co` now exists, so the *old* "authentication required" blocker is gone. The terminal now fails ONE step later: the per-Org **vCluster HelmRelease `walk-stranger-co/vcluster` (chart `vcluster@0.33.3`) install FAILED** — kyverno `harbor-proxy-pull` (`image-from-harbor-proxy-workload`) denied the vCluster StatefulSet because its initContainer image is not re-tagged through the Sovereign's local Harbor proxy (`registry.<fqdn>/proxy-ghcr/…` or `…/openova-io/…`). No vCluster pod -> no WordPress -> the terminal screenshot **still cannot be produced** on hw158. Next fix needed: pin/re-tag the vcluster chart images through the Harbor proxy glob so the StatefulSet passes the Enforce policy. |
