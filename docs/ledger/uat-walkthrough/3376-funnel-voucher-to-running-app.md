...
## Status — last validated: hw158 (2026-06-17) — browser walk: **2 ✅ / 22 ❌ / 0 GAP**
...
| [marketplace.hw158.omani.works/redeem](https://marketplace.hw158.omani.works/redeem/?code=JUNK) | Open the redeem page with a **junk code** → see a generic **"voucher not valid"** message (no tombstone / no detail leak). | ✅ | Redeem page (storefront chrome "OpenOva", wizard steps 1-6) shows the honest generic **"Voucher not valid — This code does not exist or has been retired"** with only a "Browse plans without a voucher" link. No detail leak. ![3376-b2](../../sessions/2026-06-17/evidence/3376-b2-redeem-junk-code-invalid.png) |
...
| Tested page | Description | Status | Evidence |
|---|---|---|---|
| [marketplace.hw158.omani.works](https://marketplace.hw158.omani.works/) | While still signed in, **re-open the marketplace root** in the same browser → the returning-user redirect sends the customer to **`https://console.<slug>.omani.homes`** (their own Org console), **never** to `console.openova.io` / the mothership. | ✅ | Re-opening the marketplace root while signed in redirects to **`console.hw158.omani.works/dashboard`** (this Sovereign's own console), **never** to the `console.openova.io` mothership. (Walked as the owner; a per-customer-Org redirect needs a customer Org, which doesn't exist here — but the mothership-bounce is correctly avoided.) ![3376-b17](../../sessions/2026-06-17/evidence/3376-b17-returning-user-redirect-own-console.png) |
