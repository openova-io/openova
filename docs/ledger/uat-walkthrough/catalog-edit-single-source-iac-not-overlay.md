...
>
> **POST-FIX NOTE (2026-06-17, #3749 live):** the Gitea-auth secret that A1 depends on is now **Helm-hook-provisioned**, not hand-mirrored — `secret/openova-catalog-sovereign-git-auth -n flux-system` carries `app.kubernetes.io/managed-by: Helm` + `helm.toolkit.fluxcd.io/name: bp-catalyst-platform` + `catalyst.openova.io/component: catalog-sovereign-git-auth` (chart bp-catalyst-platform@1.4.664). So the `openova-catalog-sovereign` GitRepository auth (A1 ✅) now survives a fresh re-prov without manual mirroring — that part of #3749 is confirmed durable. **The A2 single-source FAIL is UNCHANGED** (still #3668: console edit writes the wrong Gitea location, Helm co-owns the CR, editor surfaces absent) — #3749 fixed only the source-auth, not the console-edit→Flux-path wiring.
