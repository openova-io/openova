...
## Status — last validated: hw159.omani.works (2026-06-18) — browser walk: **16 ✅ / 7 ❌ / 14 GAP** (25 browser-checkable rows)
...
|---|---|---|---|
| [console.hw159/dashboard](https://console.hw159.omani.works/dashboard) | Open the console root. Expect: PIN-authenticated redirect lands directly on `/dashboard`, header shows signed in as `emrah.baysal@openova.io`. A login/PIN form = FAIL. | ✅ | Handover token minted; `FINAL_URL=…/dashboard` (no `/auth` redirect), avatar **E** top-right, 94-item treemap renders. ![signin](../../sessions/2026-06-17/evidence/hw159-signin-dashboard.png) |
| [console.hw159/dashboard](https://console.hw159.omani.works/dashboard) | Confirm the dashboard renders as the authenticated operator (sidebar nav visible: Dashboard / Organizations / Apps / Catalog). No redirect to auth. | ✅ | Full sidebar (Dashboard/Cloud/Apps/Catalog/Sandbox/Jobs/Compliance/Users/Organizations/Settings) + treemap (LAYER1 Cluster, LAYER2 Application) rendered as the authed operator. ![signin](../../sessions/2026-06-17/evidence/hw159-signin-dashboard.png) |
...
| [console.hw159/organizations](https://console.hw159.omani.works/organizations) | Open the operator Organizations directory. Expect: a card/row for the customer Organization is listed (the canonical Organization). An empty directory = FAIL. | ✅ | Directory shows **2/2 orgs**: parent `hw159.omani.works` (PARENT) + **`Acme Corp`** (the customer Org, owner admin@acme.com). The canonical customer Organization IS surfaced. ![org-dir](../../sessions/2026-06-17/evidence/hw159-3687-01-organizations-directory.png) |
| [console.hw159/organizations](https://console.hw159.omani.works/organizations) | On the customer card/row, verify the badges are correct: kind = `customer`, tier = `sme`, billing = `real`. Wrong/missing badges = FAIL. | ✅ | Acme Corp row: KIND **CUSTOMER**, TIER **Sme**, BILLING **REAL**, ISOLATION **vcluster**, STATUS **ACTIVE** — exactly the canonical customer-Org badge set. (Detail page confirms slug `acme`, owner admin@acme.com, console `console.acme.omani.homes`.) ![org-dir](../../sessions/2026-06-17/evidence/hw159-3687-01-organizations-directory.png) ![acme-detail](../../sessions/2026-06-17/evidence/hw159-3687-03-acme-detail-canonical.png) |
