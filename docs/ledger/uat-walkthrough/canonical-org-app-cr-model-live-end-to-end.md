...
|---|---|---|---|
| [console.hw158/](https://console.hw158.omani.works/) | Open the console root. Expect: PIN-authenticated redirect lands directly on `/dashboard`, header shows signed in as `emrah.baysal@openova.io` (`tier=owner`). A login/PIN form = FAIL. | ✅ | Root redirected to `/dashboard` signed-in (no login form). ![3687-p1-root-signed-in](../../sessions/2026-06-17/evidence/3687-p1-root-signed-in.png) |
| [console.hw158/dashboard](https://console.hw158.omani.works/dashboard) | Confirm the dashboard renders as the authenticated operator (sidebar nav visible: Dashboard / Organizations / Apps / Catalog). No redirect to auth. | ✅ | Dashboard rendered authed; left nav Dashboard/Apps/Catalog/Organizations/Settings visible; avatar "E". ![3687-p2-dashboard-authed](../../sessions/2026-06-17/evidence/3687-p2-dashboard-authed.png) |
...
| Tested page | Description | Status | Evidence |
|---|---|---|---|
| [console.hw158/catalog](https://console.hw158.omani.works/catalog) | Open the Blueprint catalog. Expect: a grid of Blueprint cards renders (`bp-wordpress`, `bp-postgres`, `bp-valkey`, …). Open `bp-wordpress` → **New instance**. The instance form renders with name, Org, topology fields. A blank/empty catalog = FAIL. | ✅ | Catalog renders signed-in with a full grid (64 catalog links incl. bp-postgres, bp-valkey, bp-grafana, bp-harbor, bp-keycloak, bp-gitea). Not blank. (bp-wordpress not in this catalog; New-instance form not separately opened.) ![3687-d1-catalog-newinstance](../../sessions/2026-06-17/evidence/3687-d1-catalog-newinstance.png) |
...
| [console.hw158/dashboard](https://console.hw158.omani.works/dashboard) | Scan every treemap cell for ephemeral Job pods. Expect: NO Job cell ever appears — no `cutover-*`, no `scan-vulnerabilityreport-*`, no `*-snapshot-save-*`. Any Job-named cell = FAIL. | ✅ | No ephemeral Job cells in the treemap — no `cutover-*`, `scan-vulnerabilityreport-*`, or `*-snapshot-save-*` cells observed. ![3687-e2-no-job-cells](../../sessions/2026-06-17/evidence/3687-e2-no-job-cells.png) |
| [console.hw158/apps](https://console.hw158.omani.works/apps) | Count the Application cards. Expect: one card per `Application` (NOT one per HelmRelease/pod); bootstrap-owned platform Applications carry a **Platform** badge. A card count derived from HelmReleases (≫ the canonical Application count) = FAIL. | ✅ | Apps list renders 49 cards (1-per-Application, search says "your 49 apps"); every platform card carries a **BOOTSTRAP** ("always installed") badge. Not a HelmRelease-derived explosion. ![3687-e3-apps-cards-1to1](../../sessions/2026-06-17/evidence/3687-e3-apps-cards-1to1.png) |
...
| [console.hw158/apps](https://console.hw158.omani.works/apps) | Cross-check the Apps list against the Organizations directory: every Application card maps to an Org shown in `/organizations`, and the operator console renders one consistent Organization → Application model end-to-end (Dashboard + Apps + Organizations + Showback all agree). Surfaces disagreeing on the set = FAIL. | ✅ | The Apps list (49 BOOTSTRAP/platform cards under the parent estate) is consistent with the Organizations directory (1/1 = parent only) — both surfaces agree on the single-estate, platform-only set. No customer apps/orgs to diverge. ![3687-g2-console-cross-surface-parity](../../sessions/2026-06-17/evidence/3687-g2-console-cross-surface-parity.png) |
