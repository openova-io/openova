...
## Status — last validated: hw158 (2026-06-17) — ❌ FAILED (rename NOT landed; BEFORE-baseline intact)

- **Verdict: ❌ FAILED on hw158.** The `sme`→`org-services` / `tenant`→`Organization` mechanical rename + CI naming-guard is **NOT landed** and is **not present in the repo** on this branch (`docs/uat-regen-current-env-3581`). Every "after" row (the renamed namespace, route, secret, template dir, Go identifiers, deprecation alias, CI guard) is absent; the env still runs the full `sme`-named mesh exactly as the Section-0 BEFORE baseline describes.
- **Tally: 7 ✅ / 30 ❌ / 5 N/A** (42 rows). The 7 ✅ are all **Section-0 BEFORE-baseline rows + the one legitimate `Tier: "sme"` data-value survivor (2.3) + the PR #3390 BSS/Organizations alias survivors (8.1, 8.2)** — i.e. the OLD state is fully intact, which is exactly what a not-yet-rolled rename looks like. None of them prove the rename.
...
- **Evidence (live on hw158, this walk):**
  - ns `sme` **Active 3h37m** with the full 12-deploy mesh (`admin auth billing catalog console domain ferretdb gateway marketplace notification provisioning tenant`); **NO `org-services` ns** (`Error from server (NotFound): namespaces "org-services" not found`).
  - `sme-secrets   Opaque   11` lives under its persona name; **NO `org-services-secrets`**.
  - catalyst-api env still `CATALYST_SME_JWT_SECRET → secretKeyRef name: sme-secrets`.
  - `POST /api/v1/organizations` = **HTTP 404** (`404 page not found`); `/api/v1/sme/tenants` = **HTTP 401** (`{"error":"unauthenticated"}`) with **NO `Deprecation`/`Sunset`/`Warning` header**.
  - chart dir is still `products/catalyst/chart/templates/sme-services/` (**29 files**); `org-services/` absent (`helm template … templates/org-services/org-namespace.yaml` → `could not find template …`); `sme-namespace.yaml` renders `name: sme`.
  - Go: `HandleCreateSMETenant` etc. = **27 hits**; `SMETenantProvisionStore` etc. = **76 hits**; the new `HandleCreateOrganization*` / `OrganizationProvisionStore*` identifiers = **0 hits**. Route `rg.Post("/api/v1/sme/tenants", h.HandleCreateSMETenant)` still registered (main.go:1652).
...
- **Env caveat (not a rename defect):** on hw158 `bp-keycloak` is wedged at a chart-pull error (`oci://ghcr.io/openova-io/bp-keycloak:1.4.30: not found`), cascading non-Ready across the bootstrap mesh (gitea/grafana/guacamole/sso-bridge/catalyst-platform/continuum). Section 7's "clean roll" rows are **N/A** here because the rename was never rolled — they cannot be evaluated for this ticket on a pre-rename env.
...
| # | Go to / Run | Do | Result | ☐ |
|---|---|---|---|---|
| 0.1 | terminal | `kubectl --kubeconfig /tmp/hw158-kc.yaml get ns sme` → `sme   Active   3h37m`. (And `get ns org-services` → `Error from server (NotFound): namespaces "org-services" not found`.) | ✅ — the persona-named ns is live; the target ns does not exist yet | ✅ |
| 0.2 | terminal | `kubectl … get pods -n sme` → `admin auth billing catalog console domain ferretdb gateway marketplace notification provisioning tenant` all `1/1 Running` (`domain` 1 restart, `provisioning` 3h2m). The whole Org-provisioning mesh lives in ns `sme` | ✅ — full 12-deploy mesh Running in ns `sme` | ✅ |
| 0.3 | terminal | `kubectl … get secret -n sme sme-secrets` → `sme-secrets   Opaque   11   3h37m` | ✅ — JWT bridge secret exists under its persona name | ✅ |
...
| 0.7 | repo | `git grep -rwl "sme" core/ products/ \| wc -l` → **222**; `… "tenant" … \| wc -l` → **346**. (BEFORE counts recorded; the runbook's prior "225/398" estimate is close — the residual set AFTER must collapse to only Tier enum + docs + one-release aliases.) | ✅ — BEFORE sweep counts recorded (222 `sme`, 346 `tenant`) | ✅ |
...
| 1.7 | repo | `grep -rwn "HandleCreateSMETenant\|HandleListSMETenants\|HandleDeleteSMETenant\|HandleReconcileSMETenant" …/api/` → **27 hits** (not empty). The new `HandleCreate/List/Delete/ReconcileOrganization` identifiers → **0 hits** | ❌ — SME handler identifiers present (27), Org identifiers absent (0) | ❌ |
| 1.8 | repo | `grep -rwn "SMETenantProvisionRecord\|SMETenantProvisionStore\|NewSMETenantProvisionStore" …/api/` → **76 hits** (not empty). The new `Organization*` store types → **0 hits** | ❌ — SME store types present (76), Org store types absent (0) | ❌ |
...
| # | Go to / Run | Do | Result | ☐ |
|---|---|---|---|---|
| 2.1 | repo | `git grep -rwl "sme" core/ products/` → **222 files**. These are NOT yet reduced to only the Tier enum + docs + compat aliases — they are the full live machinery (handlers, stores, chart templates dir, routes). The gate would FAIL today | ❌ — 222 `sme` file-hits = full machinery, not the legitimate residual set | ❌ |
| 2.2 | repo | `git grep -rwl "tenant" core/ products/` → **346 files**. Includes NEW-ish Catalyst machinery (`sme_tenant.go`, `tenant.yaml` deploy, `tenant_registry.go`, `/api/v1/sme/tenants` route) — not just vendored/upstream realm strings | ❌ — 346 `tenant` file-hits include live Catalyst machinery | ❌ |
...
| # | Go to / Run | Do | Result | ☐ |
|---|---|---|---|---|
| 3.1 | `https://console.hw158.omani.works/organizations/new` | the canonical `POST /api/v1/organizations` returns `HTTP 404` (`404 page not found`) on hw158; only `POST /api/v1/sme/tenants` is registered (main.go:1652). The create form cannot post to a non-existent canonical route | ❌ — `POST /api/v1/organizations` = 404; canonical route not registered | ❌ |
...
| 7.4 | terminal | `kubectl … get hr -A \| grep -v True` → wedged HRs are due to a **pre-existing env condition** (`bp-keycloak` chart-pull `oci://ghcr.io/openova-io/bp-keycloak:1.4.30: not found`, cascading to gitea/grafana/guacamole/sso-bridge/catalyst-platform/continuum/sandbox) — **NOT caused by this rename** (no rename HR was rolled). Cannot attribute a "clean rename roll" on a pre-rename env | N/A — HR wedge is a pre-existing keycloak chart-pull issue, unrelated to the (un-rolled) rename | N/A |
