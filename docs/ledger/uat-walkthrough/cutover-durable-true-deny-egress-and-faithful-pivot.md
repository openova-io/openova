...
|---|---|---|---|
| — | Open Settings → look for a **Sovereignty section** with an **"Achieve True Sovereignty"** (cutover) call-to-action. **GAP:** the live console Settings page has **no Sovereignty section and no cutover CTA** — the cutover is fired by the post-handover backend (`ReceiveTofuArchive`), not from a console button. Record the absence as the finding (no UI to trigger the cutover end-user-side). | ☐ | — |
| — | Sweep the whole console nav (Dashboard, Apps, Jobs, Domains, Team, Billing, Settings) for **any** "Sovereignty" / "cutover" / "Achieve True Sovereignty" entry. **GAP:** none exists anywhere in the live console — there is no end-user surface to initiate or own the cutover. The cutover trigger is operator-CLI / backend-only (a finding). | ☐ | — |
