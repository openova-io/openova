...
| TC-13 | CNPG pair across regions | `/app/$id` → Topology | CNPG pair placement | One CNPG cluster per region, ReplicaCluster over ClusterMesh | ◑ | walked PASS on hw128 (region-kill doc); on hw130 pending the #3317 mesh rescue roll (Phase-1 timeout left mesh 0/0; fix merged, self-applies). |
