...
| **Walk date** | **in flight** — `hw95` reached **console-200** (fixes proven) then wiped per founder directive; clean-validation prov **`hw96`** converging now (live poller: HR climbing 19→21/56, `ghcr-pull=[ghcr.io]` #3052 gate held, rtz vcluster netpol carries `flux-system` #3056 — both verified zero-touch). **TC-01–26 walk + screenshots fire on `hw96` console-200** — that is the acceptance evidence. Provisioning itself walked from the mothership console (Part 0, TC-00a/b — [evidence](../sessions/2026-06-04/evidence/)). |
| **Overall verdict** | ☐ **NOT WALKED — but the 3 final root blockers are FIXED + PROVEN on a live prov.** The recurring fresh-prov wedge was three real catalog bugs, all merged: **#3053** (cutover fired at bootstrap → registry half-pivot → 401s), **#3054** (harbor missing `external-secrets` dep + kyverno stuck-uninstall), **#3056** (vcluster isolation NetworkPolicy blocked the Flux helm-controller from the in-vcluster API → `bp-sandbox`/mgmt-apps `StateError` — the *final* blocker). **`hw95` reached `console=200` — the first prov EVER to serve** — validating all three via charts, zero hand-patch (flux-system→rtz-vcluster reachable, bp-sandbox `InstallSucceeded`, console 200 steady 14 polls). Wiped for a pristine re-validation; **`hw96`** is converging now. **Gap owned:** hw95's console-200 was verified by curl + HR/app-state, *not* a UI screenshot, before the wipe — so the visual walk evidence comes from hw96. |
