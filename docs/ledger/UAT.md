...
| **C** | New catalog pages in place | console → Apps → **Catalog** tab (+ class page `/catalog/$bp`) | New catalog pages render | ◑ (2026-06-09) — **data-GET 404 FIXED** (#3159/#3160, hw124 chart **1.4.530**): `GET /api/v1/catalog/bp-alloy` → **200** (`source:in-cluster`), `kubectl get blueprints` → **35 CRs** incl `bp-alloy` (was 14). Root cause was the empty Gitea `catalog` org + 14-CR-only seed; seed expanded to 35. **Operator UI-walk (click Catalog → class page renders) still pending** a console session (headless agent hits the PIN-login wall). |
