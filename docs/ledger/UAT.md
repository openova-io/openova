...
- **#3374 SSO — 9/13 ✅** (console·newapi·openova-flow·openova-flow-anon-denied·keycloak-admin·marketplace + **gitea·harbor·openbao flipped** via PRs #3851/#3855/#3857, **live-confirmed by direct browser_navigate** — `hw167-3374-0{6,7,8}-*-livewalk.png` show signed-in gitea dashboard / harbor projects / openbao vault) **· 4 ❌** (grafana + guacamole convergence-blocked [not SSO-config — shared-pg mesh-DNS / jdbc-seed]; pdns DB-500 fixed #3852, 2nd-OIDC-hop partial; **hubble functionally-GREEN** — lands signed-in, probe-regex false-RED, sweep-race fixed #3853).
