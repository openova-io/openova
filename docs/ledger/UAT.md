...
| SSO-powerdns-admin | `/app/bp-powerdns-admin` (instance) | Click **Open** | New tab lands in **PowerDNS-Admin** ALREADY signed in — silent OIDC, NO login form. Realm=`sovereign`. Endpoint `visibility: private` (operator-only). | ✖ | wave-6: login form renders but OIDC never lands — /oidc/login 503 (upstream timeout; gunicorn sync workers) + KC broker "Unexpected error when authenticating with identity provider" ×3. Issue filed. `docs/sessions/2026-06-12/evidence/hw130-sso-pdns-admin.png` |
