...
| TC-G1 Catalog class ≠ instance | /catalog/bp-grafana (class page) | Read the page | Shows ONLY: Blueprint header + supported-topology list + the **list of installed instances** (children) + a **New instance** button. NO single-instance tabs (Overview/Logs/Endpoints) | ☐ | — |
...
| TC-G1 Catalog class ≠ instance | catalog-click vs deployment-click | Compare the two destinations | **Different URLs / different pages** — clicking from catalog and from deployment must NEVER open the same page (the core bug) | ☐ | — |
...
| TC-G2 Multi-instance children | /catalog/bp-<x> | Read the instances list | The class page's duty: lists **all** installed instances of this Blueprint (e.g. up to 10) as child rows, each linking to its own `/app/<id>` | ☐ | — |
| TC-G3 Open = 1-click silent SSO | /app/<id> (external-facing app) | Find the action button | An **"Open"** button is present (label "Open", NOT "Launch") because the app has an external-facing URL | ☐ | — |
...
| TC-G3 Open = 1-click silent SSO | /app/<id> for a non-external app (controller/operator) | Look for the Open button | **No Open button** — only external-facing apps get it | ☐ | — |
| TC-G3 Open = 1-click silent SSO | grafana / gitea / harbor / openbao instance pages | Tap Open on each | Each lands **signed-in** per its registered SSO method (per-app login list complete — gitea/harbor/openbao present in the catalog seed) | ☐ | — |
| TC-G4 Endpoints tab editable (CRUD) | /app/<id> → **Endpoints** tab | Read the tab | The Endpoints tab is **EDITABLE** — Add / Edit / **Alias** / Delete controls visible (not read-only display) | ☐ | — |
...
| TC-G4 Endpoints tab editable (CRUD) | The PR | Watch the 3 named checks | `kyverno-admission` + `cert-manager-precheck` + `dns-conflict-precheck` go green → PR **auto-merges** | ☐ | — |
| TC-G4 Endpoints tab editable (CRUD) | Browser, ≤2 min | Open the NEW alias FQDN | Serves with **valid TLS** + **silent SSO** | ☐ | — |
| TC-G5 Per-app topology honored | /catalog/bp-<active-active app> e.g. opensearch | Install on a 2-region Sovereign | Fans out to **N active HRs, one per region** (symmetric) | ☐ | — |
| TC-G5 Per-app topology honored | /catalog/bp-<active-hot-standby app> e.g. grafana/keycloak | Install | **2 HRs: primary=active + secondary=passive**, sync replication | ☐ | — |
| TC-G5 Per-app topology honored | /catalog/bp-<active-passive app> e.g. openbao/loki | Install | Primary active + secondary **warm-standby** (async ok) | ☐ | — |
| TC-G5 Per-app topology honored | /catalog/bp-<singleton app> e.g. seaweedfs | Install | **1 HR on primary only**; UI warns "region-kill will lose this instance" | ☐ | — |
| TC-G5 Per-app topology honored | A single-region Sovereign | Install any app | Resolves to `defaults.single-region` — no fan-out | ☐ | — |
| TC-G5 Per-app topology honored | /app/<id> → **Topology** tab | Read the placement | The actual `perCluster[]` fan-out **matches the app's declared `spec.topology`** | ☐ | — |
| TC-G5 Per-app topology honored | The full matrix | Sweep every Blueprint | Each installs honoring its row (**10 hot-standby / 14 active-active / 20 active-passive / 44 singleton**); full 88-row matrix is the contract, swept after the per-class reps pass | ☐ | — |
| TC-G6 vCluster containment | /app/<id> → placement | For each app, read its vCluster | App runs **INSIDE** its assigned vCluster (mgmt/dmz/rtz), **not** the host cluster | ☐ | — |
| TC-G6 vCluster containment | dmz apps | Check cilium-gateway, coraza/WAF, stalwart, console/gitea/harbor UIs | Land in the **dmz** vCluster | ☐ | — |
| TC-G6 vCluster containment | mgmt apps | Check catalyst-api/ui, keycloak, openbao, nats | Land in the **mgmt** vCluster | ☐ | — |
| TC-G6 vCluster containment | rtz apps | Check tenant Applications + per-Org CNPG | Land in the **rtz** vCluster | ☐ | — |
| TC-G6 vCluster containment | host exceptions | Check substrate prereqs | ONLY cilium, flux, kyverno, cert-manager, ESO, CNPG-operator, crossplane, vcluster-syncers run on host | ☐ | — |
| TC-G6 vCluster containment | negative | Confirm no app on host | **No application Blueprint** runs on the host cluster (today only 2/58 contained — that's the bug) | ☐ | — |
