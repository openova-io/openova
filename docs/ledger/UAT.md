...
| SSO-powerdns-admin | `/app/bp-powerdns-admin` (instance) | Click **Open** | New tab lands in **PowerDNS-Admin** ALREADY signed in — silent OIDC, NO login form. Realm=`sovereign`. Endpoint `visibility: private` (operator-only). | 🚫 (2026-06-09) — **bp-powerdns-admin NOT installed on hw124** (HR missing); nothing to wire until it's deployed. | — |
