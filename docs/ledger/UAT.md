...
| **Region-kill** (#3375 / NS#4) | ✅ — two live kill→promote walks: hw144 `shared-pg` HA pair (RTO ≈ 4s, RPO = 0) **and** hw158 cross-region `cnpg-pair` DR pair (RTO ≈ 1.4s, RPO = 0, replica was the **synchronous** standby per #3740 / PR #3742) | [3375-topology-dr.md](uat-walkthrough/3375-topology-dr.md) · [hw158 cnpg-pair](../sessions/2026-06-17/evidence/hw158-region-kill-walk-PASS.md) |
...
| Region-kill EXECUTION — **cross-region `cnpg-pair`** DR pair | kubectl kill→promote | ✅ | **PASS on hw158** (2026-06-17, deployment `ab2135d4`) — the genuine 2-VPC cross-region pair (separate Cluster CRs `cnpg-pair-bp-cnpg-pair-primary` on me-east-215-a + `…-replica` on me-east-215-b). 5 rows seeded on region-a with the **cross-region replica as the SYNCHRONOUS standby**, region-a severed (force-delete primary pods + cordon region-a) → `kubectl patch …-replica replica.enabled=false` promoted region-b in **RTO ≈ 1.4s** (`pg_is_in_recovery` t→f) → all 5 pre-kill rows survived → **RPO = 0**, post-kill write accepted. Gated on **#3740 / PR #3742** (chart 0.2.5): before the fix the cross-region replica streamed `async` (sync went to a local region-a HA peer) so RPO was not guaranteed 0; after the fix `synchronous_standby_names = FIRST 1 ("cnpg-pair-bp-cnpg-pair-replica")` and `pg_stat_replication` shows it `sync`. ([walk proof](../sessions/2026-06-17/evidence/hw158-region-kill-walk-PASS.md)) |

Full evidence: [3375-topology-dr.md](uat-walkthrough/3375-topology-dr.md) · `docs/sessions/2026-06-15/evidence/` (hw144 shared-pg) · `docs/sessions/2026-06-17/evidence/` (hw158 cnpg-pair).
