# UAT — OpenOva Catalyst — fresh acceptance page (RESET 2026-06-09 — pending hw124)
...
|---|---|---|---|---|---|
| SSO-grafana | `/app/bp-grafana` (instance) | Click **Open** | New tab lands in **Grafana** ALREADY signed in — silent OIDC (`prompt=none&kc_idp_hint=catalyst-pin`), **NO** login form, **NO** second click. Realm=`sovereign`. Roles mapped from KC groups (`grafana-editors`→editor, `grafana-viewers`→viewer, `sovereign-admins`→admin). | ✅ (2026-06-09) — founder-confirmed (real PIN login) **+** independent agent logged-in walk on hw124 production React tree: clicked **Open** → landed in Grafana dashboard, **NO** Keycloak form, **NO** PIN. **Re-verified ✅ (2026-06-10)** post-503/post-roll: silent OIDC chain → Grafana Home dashboard signed in, user avatar present. **NB:** a re-used synthetic user (`qa-test-owner@openova.io`) hit Grafana `user.sync "unable to create user: user not found"` from a stale auth-id binding — a FRESH synthetic owner (`qa-walk-*@omani.works`) provisions cleanly; use a unique synthetic user per walk. | `docs/sessions/2026-06-10/evidence/hw124-grafana-loggedin.png` |
| SSO-gitea | `/app/bp-gitea` (instance) | Click **Open** | New tab lands in **Gitea** ALREADY signed in — silent OIDC, NO login form. Realm=`sovereign`. **Quirk:** silent delegation rides the sovereign-realm IDR `defaultProvider=catalyst-pin` (fires with OR without the hint — verified live). gitea's `openidConnect` provider 307s the browser straight to KC's authorize endpoint built from the **persisted discovery URL** (it IGNORES `--custom-auth-url` — the prior "custom-url-mapping carries the hint" note was wrong). | ✅ (2026-06-10, #3150) — **operator UAT 2026-06-09 FAIL was launch-url → gitea ROOT (renders login page).** Two stacked defects fixed (`bp-gitea` 1.2.24): (A) catalog-seed + blueprint.yaml `ui` endpoint now declares `ssoInitPath: /user/oauth2/openova-sso` → launch-url returns the OAuth-init path (live-verified on hw124: `GET /catalyst/v1/apps/bp-gitea/launch-url` → `…/user/oauth2/openova-sso`, was `…/?kc_idp_hint=catalyst-pin&prompt=none`); (B) the sso-configure Job persisted the **in-cluster** discovery URL, so gitea 307'd the browser to the unresolvable `keycloak.keycloak.svc.cluster.local` (KC is hostname-dynamic → in-cluster discovery returns INTERNAL endpoint URLs) — the Job now persists the **public** discovery URL (public-first, in-cluster fallback for the bootstrap circle). **Logged-in walk (hw124, test-session-gated owner, reverted after): navigated the launch-url → landed on the authenticated Gitea Dashboard (`qa-walk-… - Dashboard - Catalyst Gitea`), NO login form, NO "Sign in with OpenOva SSO" button.** Durable via the 1.2.24 chart roll → re-confirm operator visual after Flux reconciles. | `docs/sessions/2026-06-10/evidence/gitea-silent-sso-logged-in-dashboard.png` |
| SSO-harbor | `/app/bp-harbor` (instance, `ui` endpoint) | Click **Open** | New tab lands in **Harbor** ALREADY signed in — silent OIDC, NO login form. Realm=`sovereign`. **Quirk:** silent SSO rides Harbor's `oidc_extra_redirect_parms` config field set to `{"kc_idp_hint":"catalyst-pin"}` by the configure-oauth Job (`platform/harbor/chart`). (The 2nd `registry` endpoint is `ssoEnabled: false` — OCI clients use robot accounts, **no** Open button there.) | ◑ (2026-06-09) — launch-url → `registry.hw124…/c/oidc/login`; **wire-proven grafana-parity** (re-confirmed #3177): `curl -D-` GET → `302 → auth.hw124.omani.works/realms/sovereign/protocol/openid-connect/auth?client_id=harbor&kc_idp_hint=catalyst-pin&redirect_uri=…/c/oidc/callback`. KC client `harbor` enabled, redirectUri `registry.<fqdn>/*` ✅. **No code change needed — harbor's chain matches grafana.** (NB: `/c/oidc/login` answers GET not HEAD, so a `curl -I` probe falsely 404s — use GET.) **✅ (2026-06-09) — independent agent logged-in walk on hw124: clicked Open → landed in Harbor Projects portal signed in as `qa-test-owner@openova.io`, NO sign-in form.** Wire 302→sovereign (PR #3169); root `/`-404 fixed by HTTPRoute backend repoint `harbor-core`→`harbor` (PR #3181). **Re-verified ✅ (2026-06-10)** post-503/post-roll: silent OIDC → Harbor Projects portal signed in as `qa-walk-*@omani.works` (library project, New Project, sidebar nav), NO sign-in form. (A re-used synthetic user hit a `409 Conflict` onboarding; a fresh synthetic owner onboards cleanly.) | `docs/sessions/2026-06-10/evidence/hw124-harbor-loggedin.png` |
| SSO-openbao | `/app/bp-openbao` (instance, `api` endpoint) | Click **Open** | New tab lands in **OpenBao** ALREADY signed in via OIDC — Realm=`sovereign`, `default_role=operator`. **Quirk:** OpenBao's `api` endpoint is `visibility: private`; silent SSO does **NOT** ride a `prompt=none` URL param — the hashicorp/cap library (`builtin/credential/jwt/path_oidc.go createOIDCRequest`) builds the `auth_url` server-side and exposes **no** query-param knob, so silent login is delivered **only** by the KC realm IDR `defaultProvider=catalyst-pin` (NOT by URL hint). Must still land signed-in with no KC login form. | ◑ (2026-06-09) — launch-url → `bao.hw124…/ui/vault/auth?with=oidc` ✅; discovery URL correct (`auth.hw124.omani.works/realms/sovereign`) ✅; KC client `openbao` enabled, redirectUri `bao.<fqdn>/.../callback` ✅. **But silent SSO was wholly broken by TWO sso-configure host bugs (PR #3177):** (1) `BAO_ADDR=openbao-openbao.openbao.svc` was NXDOMAIN — live logs `wget: bad address` every 60s → reconciler never reached the server → oidc auth method never configured; (2) `allowed_redirect_uris` registered as `openbao.<fqdn>` but app serves at `bao.<fqdn>` → callback redirect_uri would fail openbao's check. Both fixed → `bp-openbao` 1.2.26. **SPA → needs operator visual after the 1.2.26 roll.** **THEN (2026-06-10, #3186 → `bp-openbao` 1.2.27):** an independent logged-in walk proved KC silently authenticates but the **callback still FAILED** with three more defects: (1) **bound_claims mismatch** — role bound to ONLY `sovereign-admins` but the realm stamps `defaultGroups /sovereign-viewers` on every operator → `claim "groups" does not match any associated bound claim values`; (2) `ssoInitPath` emitted `?with=oidc%2F` (encoded slash) → Ember UI reverted to Token form; (3) doubled-callback 400 (caused by 2). **Fixed #3186:** widen bound_claims to admins+ops+viewers, baseline `sso-operator-read` policy + external identity-group aliases (admins/ops→`sso-operator-admin`, viewers→read), `?with=oidc`, extended sso-configure-admin policy grants. **Live acceptance (hw124, test-session-gated owner, reverted after):** real PIN→KC→OpenBao code flow → callback **HTTP 200** + `client_token`; viewer → `[default,sso-operator-read]`, admin → `[default,sso-operator-admin,sso-operator-read]`. **Durable via 1.2.27 chart roll.** **✅ (2026-06-10) — independent browser logged-in walk on hw124: `/ui/vault/auth?with=oidc` → one OIDC click → landed on the OpenBao Secrets Engines dashboard (cubbyhole/, secret/), NO Keycloak form, NO Token form. Fresh synthetic owner (`qa-walk-*@omani.works`) to avoid stale-record conflicts.** | `docs/sessions/2026-06-10/evidence/hw124-openbao-loggedin.png` |

### 2.2 Tier-2 — sovereign-realm operator-facing (4) — #2744 federate-to-sovereign

| # | Tested page | Test case | What you must see | Result | Evidence |
|---|---|---|---|---|---|
| SSO-guacamole | `/app/bp-guacamole` (instance) | Click **Open** | New tab lands in **Guacamole** ALREADY signed in — silent OIDC, NO login form. Realm=`sovereign` (`sovereign-admins`→admin). | ✅ (2026-06-10) — **promoted from ◑ after a post-incident live audit:** KC admin API confirms client `guacamole` `enabled=true` + redirectUri `guacamole.hw124.omani.works/*`; `/guacamole/` serves 200; the OpenID-ext login emits `302 → auth.hw124.omani.works/realms/sovereign/.../auth?kc_idp_hint=catalyst-pin&client_id=guacamole&response_type=id_token&redirect_uri=…/guacamole/` (silent implicit flow, correct context-path redirect — survived the network-policies incident + catalyst-api roll). Backed by the independent logged-in walk below. Original ◑ context retained: launch-url → `guacamole.hw124…/guacamole/` (live CR patched); KC client `guacamole` enabled, redirectUri `guacamole.<fqdn>/*` ✅. **But silent SSO was broken by an OIDC issuer drift (PR #3177):** live OIDC env pointed at `keycloak.hw124.omani.works/realms/catalyst` — that host 404s (no HTTPRoute) AND the `catalyst` realm does not exist (only `sovereign` does); the OpenID extension `POST /api/tokens` returned 500. Fixed the `52-bp-guacamole.yaml` overlay issuer → `auth.<fqdn>/realms/sovereign` (grafana-parity). (Separate launch-url host/path `guac.`→`guacamole.` + `ssoInitPath` in catalog-seed/blueprint tracked by PR #3172.) **✅ (2026-06-10) — independent browser logged-in walk on hw124: `guacamole.hw124…/guacamole/` → OpenID extension auto-redirected to KC (silent) → landed on the Guacamole connections home signed in as `qa-walk-*@omani.works`, NO login form. id_token carried `groups:[sovereign-viewers]`.** | `docs/sessions/2026-06-10/evidence/hw124-guacamole-loggedin.png` |
| SSO-powerdns-admin | `/app/bp-powerdns-admin` (instance) | Click **Open** | New tab lands in **PowerDNS-Admin** ALREADY signed in — silent OIDC, NO login form. Realm=`sovereign`. Endpoint `visibility: private` (operator-only). | ✅ (2026-06-10) — independent browser logged-in walk on hw124: `pdns-admin.hw124…/oidc/login` → KC silent (catalyst-pin) → landed on the **PowerDNS-Admin Dashboard** signed in as `qa-walk-*@omani.works` (Zone Management + Users + API Keys nav, Sign Out), NO login form. **Took FOUR fixes (`bp-powerdns-admin` 0.1.4 + catalyst-seed `1.4.546`):** (1) catalog-seed/blueprint endpoint was missing `ssoEnabled:true`+`ssoInitPath:/oidc/login` → launch-url 409 `sso-not-enabled`; (2) served host was 2-level `admin.dns.<sov>` (NOT covered by the `*.<sov>` wildcard cert → TLS name-mismatch) → flipped slot-11a `gateway.host` to single-level `pdns-admin.<sov>` (matches the KC client redirectUris too); (3) chart OIDC env-var NAMES were wrong (`_CLAIM`/`_PROPERTY` suffixes) → PDA ignored them → `Setting().get('oidc_oauth_firstname')` fell through to DB default `given_name` → unguarded `user_data['given_name']` → HTTP 500 callback; renamed to `OIDC_OAUTH_USERNAME/_FIRSTNAME/_LAST_NAME/_EMAIL`; (4) catalyst-pin carries no name claims → firstname+last_name point at `preferred_username`. (Separate, NON-SSO: the empty zones table — `PDNS_API_URL` dialed `bp-powerdns-api.powerdns.svc` (NXDOMAIN, no such svc) — **FIXED #3207 (chart 0.1.6): point at the real `powerdns.powerdns.svc:8081`. Live-verified: pod Ready/0-restarts, API `/api/v1/servers` → 200.**) | `docs/sessions/2026-06-10/evidence/hw124-powerdns-admin-loggedin.png` |
| SSO-netbird | `/app/bp-netbird` (instance) | Click **Open** | New tab lands in **NetBird** dashboard ALREADY signed in — silent OIDC, NO login form. Realm=`sovereign` (`sovereign-admins`→admin). | 🚫 (2026-06-09) — **bp-netbird NOT installed on hw124** (HR missing; historically deferred per memory); deploy it first. | — |
...
| # | Tested page | Test case | What you must see | Result | Evidence |
|---|---|---|---|---|---|
| TOPO-powerdns-admin | `/app/bp-powerdns-admin` → Topology | Read placement | **singleton** → 1 HR (admin UI state in PG); a cluster failure means edit via another cluster's instance; no cross-cluster sync. | ◑ (2026-06-10) — **placement data verified live (kubectl):** exactly **1 HR** `flux-system/bp-powerdns-admin` Ready (chart 0.1.6, #3207), pod `powerdns-admin-bp-powerdns-admin-85f756d56-2mmpv` on node `…me-east-215-a-w1e5887` — single HR on the single cluster = singleton by construction (admin state in CNPG `pda-pg`, 1 instance). Matches the declared singleton topology. **Topology-tab UI render pending** an operator PIN session. | kubectl: `get hr -n flux-system bp-powerdns-admin` + `get pods -n powerdns-admin -o wide` |
...
| TOPO-seaweedfs | `/app/bp-seaweedfs` → Topology | Read placement | **singleton** (per cluster) → 1 HR primary; within-cluster replicas + erasure coding; cross-cluster is async pull only — **region-kill-loses-it** warning if not replicated. | ◑ (2026-06-10) — **placement data verified live (kubectl):** exactly **1 HR** `flux-system/bp-seaweedfs` Ready; 7 within-cluster pods on the single me-east-215-a cluster (master/volume/filer within-cluster replicas) — singleton-per-cluster, no cross-cluster sync. Matches the declared topology. **Topology-tab UI render pending** PIN session. | kubectl: `get hr bp-seaweedfs` (1, Ready) |
...
| TOPO-cilium | `/app/bp-cilium` → Topology | Read placement | **singleton** (per cluster) → each cluster owns its identity space; ClusterMesh shares endpoint reachability only, not control state. | ◑ (2026-06-10) — **placement data verified live (kubectl):** exactly **1 HR** `flux-system/bp-cilium` Ready; cilium DaemonSet 4/4 (one agent per node, the per-cluster CNI) on the single me-east-215-a cluster — singleton-per-cluster owning its own identity space. Matches the declared topology. **Topology-tab UI render pending** PIN session. | kubectl: `get hr bp-cilium` (1, Ready) + `get ds cilium -n kube-system` (4/4) |
