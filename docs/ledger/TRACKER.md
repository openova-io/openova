...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T08:54:00Z` (🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢 **70 PRs merged + 26 Wave issues CLOSED + Wave 5.36→5.61 chain LIVE on hw01**: **Wave 5.57c #2314** chart 1.4.45→1.4.46 — `checksum/sandbox-bridge` Pod annotation forces rollout on any sandboxBridge values change (closes diff-equivalence gap that left hw01 Pod running old 1.4.44 spec). **Wave 5.61 #2313** — catalyst-api HPA-down DEFENSE (min=1/max=1 default, operator-overridable; #2306 follow-up). **Wave 5.60 #2312** — CI smoke test for bp-openbao fresh-prov self-heal (DEFENSE for #2295). **Wave 5.59 #2308** — catalyst-api mem request 192Mi→96Mi (mothership Pod Pending 48+min on 99% mem-requests substrate; live-patched + verified Running). **#2306 CLOSED**. **Wave 5.57b #2309** — bp-sandbox 0.3.6→0.3.7 (newapiBaseURL :3000→:8080 publish). **Wave 5.55 #2310** — bp-openbao init activeDeadlineSeconds 600s→1200s. **Wave 5.57 #2304** — sandbox-bridge sidecar (cmd/main.go + Dockerfile + CI + chart sidecar + Service bridge:8080 + NetPol). **Wave 5.57a #2301** — NetPol catalyst-system. **#2303 walk-verification**: L7 progressed (pre `<`HTML → post `:8080 context deadline`); awaits hw01 Flux convergence of chart 1.4.46 (~09:05Z). EPIC #1094 ACHIEVED. 3 EPIC umbrellas (#1094 #1096 #1099) remain open. Supervisor coaches resolved: P21 (×13), P22 (×35), P26 (×4), P23/D16 (×1), §1 (×1), D14 (×4).) |
