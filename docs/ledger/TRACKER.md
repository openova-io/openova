...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T07:52:00Z` (🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢 **63 PRs merged + 25 Wave issues CLOSED + Wave 5.36→5.57 chain LIVE on hw01**: **Wave 5.57 #2304** — sandbox-bridge sidecar shipped. Pre-fix: `platform/newapi/internal/handler` was orphan Go code (handler + tests + go.mod but no main.go/Dockerfile/CI) → upstream Calcium-Ion binary returned HTML 404 → `TokenMintFailed: decode response: invalid character '<'` on hw01 walk 2026-05-24 07:43Z. Fix ships: `cmd/sandbox-bridge/main.go` exec + Dockerfile (distroless static nonroot) + `.github/workflows/build-bp-newapi-sandbox-bridge.yaml` CI + chart sidecar + Service `bridge:8080` port + NetPol 8080 + sandbox chart `newapiBaseURL:3000→8080`. CI in flight; chart auto-bumps; hw01 Flux reconciles next tick. **Wave 5.57a #2301** chart 1.4.43→1.4.44 — NetworkPolicy allows catalyst-system alongside traefik (walk-validated: `<` HTML body proves L3 reach OK, only L7 endpoint contract gap which 5.57 closes). EPIC #1094 'half enforcing' ACHIEVED. 3 EPIC umbrellas (#1094 #1096 #1099) remain open per long-running convention. **Mothership substrate alert**: catalyst-api Pod Pending 16+ min (Insufficient memory on single 12GB VM) — high-ICE substrate gap. Supervisor coaches resolved: P21 (×11), P22 (×31), P26 (×4), P23/D16 (×1), D14 (×4).) |
