...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T07:35:00Z` (🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢 **61 PRs merged + 25 Wave issues CLOSED + Wave 5.36→5.57a chain LIVE + 52 HRs Ready=True on hw01**: Wave 5.57a #2301 chart 1.4.43→1.4.44 — bp-newapi NetworkPolicy now allows ingress from catalyst-system (sandbox-controller's `/admin/tokens/sandbox` mint path) alongside traefik. helm-template verified BOTH selectors render under ingress[0].from[]. GHCR has bp-newapi:1.4.44 (auto-bumped on merge). Earlier: bp-openbao + bp-newapi + bp-sandbox ALL UNBLOCKED via kubectl exec bao init + ghcr-pull mirror + Wave 5.56 SOVEREIGN_REGION_CANONICAL_LABEL substitute (#2297). sandbox-controller pod 1/1 Running. Pillar 4 backend infra LIVE end-to-end. **Mothership substrate alert**: catalyst-api Pod Pending 16+ min (Insufficient memory on single 12GB VM) — orchestrator effectively down for new provs + fleet-view reach to hw01. Walk verification of 5.57a defers to Playwright drive on console.hw01 public surface (HCS firewall blocks 6443 from bastion). EPIC #1094 'half enforcing' ACHIEVED. 3 EPIC umbrellas (#1094 #1096 #1099) remain open per long-running convention. Supervisor coaches resolved: P21 (×11), P22 (×29), P26 (×4), P23/D16 (×1), D14 (×4).) |
