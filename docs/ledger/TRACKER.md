...
## 🔧 Manual session update — 2026-06-09 (generator stale since 2026-06-03; ran it, no-op → cron dead)

**EPIC #3145 — cloud-agnostic Sovereign bootstrap:**
- **MERGED:** #3146 (ONE shared cloud-init, PUT-early; −1747 lines drift) · #3147 (stage `_shared` sibling into tofu workdir + structural guard) · #3149 (kyverno webhook-gate **non-fatal** — stops the fatal-hook→stuck-uninstall→`console 000` cascade) · #3151 (janitor must not reap the `_shared` staging dir) · #3152 (provider-agnostic provisioning-job labels — was hardcoded "Provision Hetzner")
- **FILED:** #3150 (silent-SSO broken at 3 layers for bootstrap-kit apps: launch-url base-URL not OIDC-init path, `auto_login:false`, HR apps get no launch-url) · #3153 (provisioning **visibility** lost during convergence — mothership rolls mid-prov wipe the in-memory Phase-1 event stream + resume-after-restart gap)
- **#2989 RESOLVED:** cnpg exit-2 = cluster-wide kubelet Secret-projection wedge from the k3s **sqlite** datastore under convergence load; **etcd (`--cluster-init`) clears it, A/B-proven** (hw118 sqlite hung 2min+ vs hw123 etcd 50s).
- **IN-PROGRESS:** hw124 (`90a98a78`) clean re-prov — **converged zero-touch past the 31/56 #2989 wedge wall** (48/56 at writing), first prov to clear it with no manual nudges. UAT (`docs/ledger/UAT.md`): TC-01/05/06/10 walked on hw123 (since wiped clean); full walk pending on hw124. #3129 cloud-init saga superseded/resolved by #3146.
...
2026-06-08 21:00Z auto-tick | main@6400d395 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-08 21:00Z auto-tick | main@87af4481 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-08 21:00Z auto-tick | main@92b354bd | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-08 21:00Z auto-tick | main@57fd1449 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-08 21:00Z auto-tick | main@da4ed1dd | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-08 21:00Z auto-tick | main@f2bb8589 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-08 21:00Z auto-tick | main@75374823 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-08 21:00Z auto-tick | main@dc2f6017 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-08 21:00Z auto-tick | main@d543b5b5 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-08 21:00Z auto-tick | main@9af514c5 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
