...
## 🟦 G117 batch1 follow-ups (2026-06-02) — perClusterFanout wiring + KUBECONFIG export + per-Org Gitea token + Tempo port

Single PR closes four W1/W2 verifier follow-ups in one shot:

- **G117.6a ([#2796](https://github.com/openova-io/openova/issues/2796))** — `application_controller.go`'s `_ = perClusterFanout` discard replaced with a real HR persistence loop that calls `upsertHostResource(FluxHelmReleaseGVR, ...)` per cluster in `Variant.Placement.Clusters[]`. `FanoutInputs.KubeConfigSecretFor` is populated from `VClusterPlacements[Placement.Tier]` so each per-cluster HR carries `spec.kubeConfig.secretRef.name = vc-<cluster-lowered>` (G92.1 #2674 pivot). New `application_controller_fanout_persist_test.go` asserts 2-cluster active-hot-standby produces 2 HRs in the mgmt host namespace with distinct kubeConfig secrets + correct role labels, plus an idempotency assertion.
- **G117.3b ([#2765](https://github.com/openova-io/openova/issues/2765))** — writer-side completes the ADR-0009 round-trip. New `Handler.NewProductionGiteaIaCWriter(ctx, org)` + `resolveGiteaTokenForOrg(ctx, org)` read from OpenBao at `kv/data/org/<slug>/iac-bot-token` (key `token`) via the new `openbao.Client.GetKVv2` method. Transport-error vs not-found is honoured: 404 falls back to the global `CATALYST_GITEA_TOKEN` env shim; 5xx propagates. main.go's `WriterFactory` wrapped in a per-call closure so the openbao path runs in production. 6 new unit tests in `endpoint_handler_per_org_token_test.go` (cross-Org isolation + 404 fallback + transport-error propagation + no-openbao-wired + empty-org rejection).
- **G117.3c follow-up ([#2797](https://github.com/openova-io/openova/issues/2797))** — `test-bootstrap-api-integration.yaml` gains an explicit `Export KUBECONFIG` step after `helm/kind-action@v1.10.0` so the canary check at the integration-test step actually sees the env var. Without this the integration tests have been silently-skipping in every run since the workflow shipped (caught by the W2.C5 PR #2792 verifier).
- **G117.5a ([#2788](https://github.com/openova-io/openova/issues/2788))** — bp-grafana 1.0.5 → 1.0.6: Tempo datasource URL `http://tempo.tempo.svc.cluster.local:3100` → `:3200` (upstream Tempo single-binary chart's canonical HTTP query port; 3100 was a copy-paste mismatch from Loki).

Refs [#2796](https://github.com/openova-io/openova/issues/2796) [#2797](https://github.com/openova-io/openova/issues/2797) [#2772](https://github.com/openova-io/openova/issues/2772) [#2765](https://github.com/openova-io/openova/issues/2765) [#2788](https://github.com/openova-io/openova/issues/2788) [#2737](https://github.com/openova-io/openova/issues/2737). Branch: `fix/g117-followups-batch1`. Drive-by: fixed a pre-existing syntax error in `endpoint_handler_test.go` (missing `})` between two test functions) that was blocking the entire handler test suite from compiling.
...
2026-06-02 11:30Z auto-tick | main@6400d395 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-02 11:30Z auto-tick | main@87af4481 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-02 11:30Z auto-tick | main@92b354bd | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-02 11:30Z auto-tick | main@57fd1449 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-02 11:30Z auto-tick | main@da4ed1dd | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-02 11:30Z auto-tick | main@f2bb8589 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-02 11:30Z auto-tick | main@75374823 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
...
2026-06-02 11:30Z auto-tick | main@dc2f6017 | n/a | Cron heartbeat — no manual TRACKER row since last main commit; refresh-openova-tracker.sh fired. If you see consecutive auto-ticks, the session is silent — confirm work is happening or pause cleanly. |
