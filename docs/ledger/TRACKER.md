...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T01:22:00Z` (🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢 **bp-opentelemetry:1.1.2 PUBLISHED to GHCR — EPIC #1094 OTel-auto-instr gap CLOSED via 4-PR chain (Wave 5.36 #2210 + 5.37 #2217 + 5.38 #2220 + 5.39 #2222).** Schema-shape iteration: 5.36 shipped with `resources` at wrong level (schema rejected at root) → 5.38 moved both `resources` + renamed `replicaCount`→`replicas` under manager (schema rejected `replicas` under manager) → 5.39 final correct shape (`replicaCount` at root + `resources` under `manager.*`). Wave 5.37 Kyverno wildcard fix unblocks compliance evaluation of cross-NS Instrumentation references. Plus prior turn: 37 UAT closures on hw01.omani.works fresh prov 102ac1f980d6fc7f (Huawei HCS). Pillars 1-5 walks (P1 vouchers, P2 BCP wizard, P3 CNPG pair CR, P4 Sandbox 11-manifest GitOps, P5 cutover 9-step chart). 22 UAT closed with Playwright/kubectl/API evidence: #2204/2201/2199/2197/2195/2193/2191/2188/2186/2184/2181/2178/2176/2174/2171/2168/2166/2164/2085/2131/2128/1808.) |
| Open issues | 3 — EPIC umbrellas (#1099 Cloud Resources, #1096 Compliance, #1094 Phase 0/1) all carry shipped-delta comments; closed only when all sub-items done |
| Open DoD gates | 7 / 41 |
...
| 5.36 | [#2210](https://github.com/openova-io/openova/pull/2210) (MERGED 2026-05-24T01:01Z) | <img alt="DONE" src="https://img.shields.io/badge/-DONE-2ea043?style=flat-square" /> | bp-opentelemetry 1.0.0 → 1.1.0 — add opentelemetry-operator 0.62.0 subchart + default Instrumentation CR template (per-language SDK pins Go/Java/Python/NodeJS/.NET) + cert-manager-issued webhook certs. Closes the documented EPIC #1094 "OTel Operator + auto-instrumentation (no Instrumentation CRs)" gap. Per-Wave issue #2215. |
| 5.37 | [#2217](https://github.com/openova-io/openova/pull/2217) (MERGED 2026-05-24T01:09Z) | <img alt="DONE" src="https://img.shields.io/badge/-DONE-2ea043?style=flat-square" /> | bp-kyverno-policies 1.0.0 → 1.0.1 — `15-otel-injected.yaml` ClusterPolicy: 5 anyPattern branches `"true"` → `"?*"` (Kyverno wildcard, any non-empty). Was emitting false-negatives against the canonical OTel Operator cross-NS form `"opentelemetry/default"` that Wave 5.36 enabled. EPIC #1096 compliance correctness. Per-Wave issue #2216. |
| 5.38 | [#2220](https://github.com/openova-io/openova/pull/2220) (MERGED 2026-05-24T01:16Z; CI still failed — over-corrected) | <img alt="DONE" src="https://img.shields.io/badge/-DONE-2ea043?style=flat-square" /> | bp-opentelemetry 1.1.0 → 1.1.1 — over-corrected: moved `resources` AND renamed `replicaCount`→`replicas` under `manager.*`. Upstream schema then rejected `manager: Additional property replicas is not allowed`. Superseded by Wave 5.39. Per-Wave issue #2219. |
| 5.39 | [#2222](https://github.com/openova-io/openova/pull/2222) (MERGED 2026-05-24T01:21Z; **bp-opentelemetry:1.1.2 PUBLISHED**; #2221 CLOSED via 5-step UAT cycle) | <img alt="DONE" src="https://img.shields.io/badge/-DONE-2ea043?style=flat-square" /> | bp-opentelemetry 1.1.1 → 1.1.2 — correct final shape: `replicaCount: 1` at root of `opentelemetry-operator:` subchart + `resources:` under `manager.resources`. Blueprint Release CI passed; chart published to GHCR at sha256-7464b1bf. Per-Wave issue #2221 CLOSED (reviewer-concur VERIFIED-PASS at chart layer + PARTIAL at hw01 layer — chart-layer DoD met). |
| 5.40 | [#2226](https://github.com/openova-io/openova/pull/2226) (MERGED 2026-05-24T01:37Z; **bp-opentelemetry:1.1.3 PUBLISHED but hw01 still rolled back**) | <img alt="OPEN" src="https://img.shields.io/badge/-OPEN-cf222e?style=flat-square" /> | bp-opentelemetry 1.1.2 → 1.1.3 — added `helm.sh/hook: post-install,post-upgrade` weight 10 on Instrumentation CR template. Did NOT fully unblock — Helm rolled back hw01 to 1.0.0 because RENDER-time validation still fails when CRD absent (hook only re-orders APPLY). Per-Wave issue #2224. Superseded by Wave 5.41 chart-split approach. |
| 5.41 | (filed, PR pending) | <img alt="OPEN" src="https://img.shields.io/badge/-OPEN-cf222e?style=flat-square" /> | **Chart split** — bp-opentelemetry-operator (new, slot 19b) for Operator + CRDs, bp-opentelemetry (slot 20, collector + CR only) with Flux dependsOn ordering. Matches bp-cnpg / bp-cnpg-pair canonical K8s pattern. Per-Wave issue #2227. Unblocks #2215 once landed. |
