...
| Open issues | 5 in-progress — #1094, #1096, #1099, #2370 Wave 5.69 (consoleUI.sidebarEntry for bp-chepherd), #2371 Wave 5.70 (public brand-kit). #2337 UAT. **Chepherd-lift contract LOCKED 11:43Z**: tag `pty-server-handoff-1.0` @ c65dbdca pushed for chepherd LICENSE attribution + `docs/INTEGRATION-OPENOVA-MCP.md` spec shipped (PR #2372) covering sidecar pattern + env contract + mcp.json + tool namespaces + auth chain + PVC + NetPol + versioning. (a)-(k) all answered on #2316. **Total: 51 PRs merged + 11 issues closed + 22 walks + 2 platform code-fixes + 1 spec doc.** |
| Open DoD gates | 7 / 41 |
