...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T09:17:00Z` (🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢 **71 PRs merged + 28 Wave issues CLOSED + Wave 5.36→5.62a chain LIVE on hw01**: **Wave 5.62a #2319** — Compliance nav entry in SovereignSidebar.tsx (exposes existing /sre/compliance + /sec/compliance dashboards via main sidebar; backend pipeline + page components already shipped under /admin/compliance/* — gap was nav-exposure only). #2318 → status/uat. #2317 closed as superseded by #2320 closed as duplicate-of-shipped (ComplianceTab already in AppDetail.tsx:581). #1099 audit corrected: K8sListPage + ResourceDetailPage (903 LOC) + PodLogsPage + ExecPanel (xterm + Guacamole) + SBOMTab all shipped in products/catalyst/bootstrap/ui — earlier audit grepped wrong UI surface (core/console has 11 Svelte comps; real surface is the React SPA). **Wave 5.57c #2314** chart 1.4.45→1.4.46 `checksum/sandbox-bridge` annotation. **Wave 5.61 #2313** catalyst-api HPA-down. **Wave 5.60 #2312** bp-openbao fresh-prov CI smoke. **Wave 5.59 #2308** catalyst-api mem 192Mi→96Mi (#2306 CLOSED + live-patched). **Wave 5.57b #2309** bp-sandbox :3000→:8080. **Wave 5.55 #2310** bp-openbao init 600s→1200s. **Wave 5.57 #2304** sandbox-bridge sidecar. **Wave 5.57a #2301** NetPol catalyst-system. **#2303 walk-verification**: alternates PENDING ↔ TokenMintFailed `:8080 context deadline` since 08:46Z (chart 1.4.46 merged), past 30-min Flux propagation budget — gap is either Flux source-controller stuck OR sidecar runtime issue (hw01:6443 unreachable from bastion + mothership = no diagnostic path without founder hw01 access). Chepherd lift proposal for Sandbox sidecar review in flight. EPIC #1094 ACHIEVED. 3 EPIC umbrellas (#1094 #1096 #1099) remain open. Supervisor coaches resolved: P21 (×15), P22 (×37), P26 (×4), P23/D16 (×1), §1 (×1), D14 (×4).) |
