...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T01:15:00Z` (🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢 **Wave 5.38 bp-opentelemetry 1.1.1 schema fix (PR #2220 Refs #2219) — moves opentelemetry-operator `resources` + `replicas` under `manager.*` per upstream schema; Blueprint Release CI failed on 1.1.0 with "Additional property resources is not allowed", blocking GHCR publish + hw01 1.1.0 rollout.** Wave 5.37 Kyverno otel-injected accepts any non-empty inject-* value (PR #2217 MERGED) — closes EPIC #1096 false-negative gap. Wave 5.36 OTel Operator + default Instrumentation CR shipped (PR #2210 MERGED). Plus prior turn: 37 UAT closures on hw01.omani.works fresh prov 102ac1f980d6fc7f (Huawei HCS). Pillars 1-5 walks (P1 vouchers, P2 BCP wizard, P3 CNPG pair CR, P4 Sandbox 11-manifest GitOps, P5 cutover 9-step chart). 22 UAT closed with Playwright/kubectl/API evidence: #2204/2201/2199/2197/2195/2193/2191/2188/2186/2184/2181/2178/2176/2174/2171/2168/2166/2164/2085/2131/2128/1808.) |
...
| 5.38 | [#2220](https://github.com/openova-io/openova/pull/2220) | <img alt="OPEN" src="https://img.shields.io/badge/-OPEN-cf222e?style=flat-square" /> | bp-opentelemetry 1.1.0 → 1.1.1 — move `resources` + `replicas` under `manager.*` per upstream schema (Wave 5.36 placed them at root → Blueprint Release CI failed `Helm template smoke render` with "Additional property resources is not allowed" → 1.1.0 never published to GHCR). Per-Wave issue #2219. Unblocks bp-opentelemetry GHCR publish + hw01 rollout for EPIC #1094 closure. |
