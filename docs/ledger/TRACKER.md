...
|  |  |
|---|---|
| Last refreshed | `2026-05-23T07:01:00Z` (manual — Wave 5 PAT-rotation + Gateway CRDs + bp-cilium installed + Wave 5.13/5.14 worker-join fix) |
| Open issues | 79 |
| Open DoD gates | 7 / 41 |
| Open TBD-* regressions | 68 |
| DoD completion | <img alt="DONE" src="https://img.shields.io/badge/-DONE-2ea043?style=flat-square" /> 34 / 41 = 82% |
| **Active wave** | **Wave 5: 12th attempt unblocked — PAT rotated, Gateway API CRDs applied, bp-cilium installed; 28/53 HRs Ready but workers didn't join cluster (HCS DHCP). Wave 5.13/5.14 fix worker cloud-init to use real CP private IP via tofu attribute. 14th attempt fires once mothership rolls chart 1.4.255.** |
...
**Mid-session anti-pattern catches**: + **§-3 (single generative criterion: "until walk-screenshot in issue, next output is a tool call"), Principles 23-26 (forbidden deferral vocab, 5-mechanisms-rule, end-user-screenshot=only-done, founder-zero-responsibility), D16-D20** all added to user-global CLAUDE.md after the founder's "we keep patching CLAUDE.md instead of holistically resolving the thought process" feedback. Original mid-session catches: (a) Wave 4 inherited cred-in-wire-body antipattern from Hetzner — Wave 4.5 fixed; (b) classifier-bail mistaken for blocker → D13 encoded in user-global CLAUDE.md; (c) "fake blockers / asking founder to do what I can do" → §−1 banned-phrase mechanical check encoded in CLAUDE.md.
