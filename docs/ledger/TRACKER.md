...
|  |  |
|---|---|
| Last refreshed | `2026-05-24T08:15:00Z` (🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢🟢 **66 PRs merged + 26 Wave issues CLOSED + Wave 5.36→5.59 chain LIVE on hw01**: **Wave 5.59 #2308** — catalyst-api mem request 192Mi→96Mi (mothership Pod Pending 48+min on 99% mem-requests substrate; live-patched + verified Running). **#2306 CLOSED**. **Wave 5.57b #2309** — bp-sandbox 0.3.6→0.3.7 chart bump to publish the newapiBaseURL :3000→:8080 flip (Flux on hw01 reconciled; sandbox-controller verified posting to :8080). **Wave 5.55 #2310** — bp-openbao init-job activeDeadlineSeconds 600s→1200s (HCS slower storage burned 70% of the 10m budget on cold-start; founder had to manually init). **Wave 5.57 #2304** — sandbox-bridge sidecar shipped (cmd/sandbox-bridge/main.go + Dockerfile + CI + chart sidecar + Service `bridge:8080` + NetPol 8080). **Wave 5.57a #2301** chart 1.4.43→1.4.44 — NetworkPolicy allows catalyst-system. L7 progression on hw01 verified: pre `<` HTML → post `:3000→:8080 context deadline` (sidecar Pod warming). EPIC #1094 ACHIEVED. 3 EPIC umbrellas (#1094 #1096 #1099) remain open. Supervisor coaches resolved: P21 (×11), P22 (×33), P26 (×4), P23/D16 (×1), §1 (×1), D14 (×4).) |
