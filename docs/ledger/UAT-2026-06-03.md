...
> These are code-verified on `main` already (independent audit: **8/8 present, zero theater**). Each is a one-line confirm.

| # | Hardening | Confirm command | Expect |
|---|---|---|---|
| 7.1 | Sandbox RBAC (#2952) | `grep -c 'apps.openova.io' core/controllers/sandbox/internal/gitops/manifests.go` | ≥1 |
| 7.2 | Patch-script reload not masked (#2956) | `grep -c '\|\| true' tools/patch-existing-nodes-registries-yaml.sh` on the reload line | 0 |
| 7.3 | Switchover timeouts (#2957) | `cd core/controllers/continuum && go test ./internal/switchover/...` | ok |
| 7.4 | bp-vpa Cilium trap (#2946) | `helm template platform/vpa/chart --set vpaOverlay.networkPolicy.enabled=true \| grep -vE '^\s*#' \| grep -c ipBlock` | 0 |
| 7.5 | KC master Secret keep (#2945) | `grep -c 'helm.sh/resource-policy: keep' platform/keycloak/chart/templates/configmap-sovereign-realm.yaml` | ≥1 |
| 7.6 | Voucher entropy + rate-limit (#2953/#2954) | `cd core/services/billing && go test ./handlers/...` | ok |
| 7.7 | Harbor mirror token, all 4 cloud-inits (#2966) | `grep -l harbor_robot_token infra/providers/*/cloudinit-*.tftpl \| wc -l` | 4 |
| 7.8 | Catalog-seed lockstep (#2926/#2927) | `bash scripts/check-catalog-seed-lockstep.sh` | PASS |
