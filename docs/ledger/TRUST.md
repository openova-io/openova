...
| **G117.2** — Catalog drill-down (class/instance split) + multi-instance Application CRD (#2741) | 🟡 VERIFIED-PARTIAL | W1.B5 #2750 (Svelte scaffold) shipped console UI + LaunchButton + TopologyPicker + EndpointsTab but the scaffold lives at `products/catalyst/console/` and NEVER reached operators — the catalyst-ui Containerfile copies only the React tree at `products/catalyst/bootstrap/ui/`. V2 brutal verifier caught this 2026-06-03; closure was theater. Fixed by PR #2876 (React port of class-detail page at `/catalog/$blueprintName`) + PR #2877 (TS dedupe). VERIFIED LIVE 2026-06-03 on `catalyst-ui:edab3c1` — bundle `index-X_vV51lZ.js` contains 8 `catalog-drilldown` test-ids + 2 `btn-new-instance` hits; unauth probe of `/catalog/grafana` correctly 302s to `/login?next=/catalog/grafana` (was HTTP 200 + "Not Found" before fix). Visual class-detail render of the page still needs founder PIN-walk. W2.C2 #2795 CRD changes remain CODE-COMPLETE. |
...
| **G117.4** — Launch button + silent SSO (#2743) | 🟡 VERIFIED-PARTIAL | W1.B5 #2750 shipped LaunchButton.svelte + buildLaunchURL per locked decision #3. V4 brutal verifier 2026-06-03 caught TWO live bugs in the React UI (the bundle that actually ships to operators, not the Svelte scaffold): (1) `LaunchURLResponse { URL: string }` interface vs Go `json:"url"` — `resp.URL` always undefined → fallbackURL always taken → silent SSO never fired; (2) per-card "Open" button in AppsPage.tsx opened raw externalURL bypassing silent SSO (founder's exact complaint). Fixed by PR #2873. VERIFIED LIVE 2026-06-03 on `catalyst-ui:edab3c1` — bundle inspection confirms `(await Ok(e))?.url??t` (lowercase) in LaunchButton function `GDe`; `open-app-btn` + `sov-app-open-` grep counts = 0. Visual `window.open(silentSSOURL)` click-walk needs founder PIN-walk. |
...
## 2026-06-03T21:40Z — G117 V-verifier round 2 (post-reopen) — 5 PRs shipped, 2 sub-EPICs flipped to 🟡 VERIFIED-PARTIAL

Founder reopened 13 G117 issues 2026-06-03 demanding brutal revalidation after catching premature closures on theater patterns. V1–V13 brutal verifiers dispatched; 10/13 verdicts said "keep open / theater caught" and 3/13 said "genuine pass". Code-side fix-up follow-ups merged this session:

| PR | Issue | What | Status |
|---|---|---|---|
| #2873 | #2743 | Launch URL casing `URL` → `url` + remove per-card Open button | MERGED + LIVE on `catalyst-ui:edab3c1` |
| #2874 | #2873-followup | Unblock catalyst-build CI (TS6133 unused-local + PinInput6 stale tests) | MERGED + LIVE |
| #2875 | #2870-followup | Drop duplicate `Namespace/velero` from slot 34a (froze bootstrap-kit Kustomization repo-wide; blocked all chart-pin updates) | MERGED + LIVE |
| #2876 | #2741 | Catalog class drill-down React port (`/catalog/$blueprintName` route + `CatalogDetail.tsx`) — ports Svelte scaffold to production React UI | MERGED |
| #2877 | #2876-followup | Dedupe `ListBlueprintInstancesResponse` TS interface (TS2395) | MERGED + LIVE |
