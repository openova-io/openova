...
Multiple guards run on every chart change. The pre-merge guard is the **primary** line of defence; the post-merge guards are belt-and-braces structural verification at publish time.

| When | Workflow | Guard | Failure mode caught |
|---|---|---|---|
| **Pre-merge (PR) — GUARD 1 (no-upstream)** | `.github/workflows/check-chart-annotations.yaml` → `scripts/check-chart-annotations.sh` | For every changed `platform/*/chart/Chart.yaml` or `products/*/chart/Chart.yaml`: chart MUST EITHER declare a non-empty `dependencies:` block OR carry the annotation `catalyst.openova.io/no-upstream: "true"`. | The hollow shape itself, **before the chart version is dead-reserved by a failed publish.** Three real recurrences pre-2026-05-20 each needed a follow-up version-bump PR; pre-merge catches the violation while the version in `Chart.yaml` can still be edited in place. |
| **Pre-merge (PR) — GUARD 2 (smoke-render)** | `.github/workflows/check-chart-annotations.yaml` → `scripts/check-chart-annotations.sh` | For every changed chart: `helm template` with default values must produce ≥5 lines OR the chart must carry the annotation `catalyst.openova.io/smoke-render-mode: "default-off"`. | Charts that render zero/empty at defaults (`enabled: false` master gate) without the opt-out annotation. **Catches the dual-annotation gap** — bp-network-policies:1.0.1 (2026-05-20) had `no-upstream:true` but missing `smoke-render-mode:default-off`, passed GUARD 1 post-merge but tripped the smoke-render check, dead-reserving the version. Elevated to pre-merge in TBD-V38. |
...
| Always | `.github/workflows/blueprint-release.yaml` smoke | `helm template` smoke render with default values produces non-trivial output OR `catalyst.openova.io/smoke-render-mode: "default-off"` is set; rendered manifests uploaded as workflow artifact for forensics. | Render-broken templates, schema violations, missing required values. Same logic as the pre-merge GUARD 2; kept here as belt-and-braces. |
