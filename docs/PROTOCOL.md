...
1. **Name the env.** CP-2 and Lane B run on **the same, explicitly named env**: the best surviving converged env as re-derived at Step 0 (as of the 2026-07-14 snapshot that is **hw250**; the current production Sovereign — hw240 as of writing — is on the §5.0 protect-list and is NEVER a CP-2 target). The name is written into the CP-2 train manifest (`docs/sessions/<date>/train-<env>.md`) before the trigger.
...
| Current production Sovereign | as of 2026-07-14: **hw240** (Step 0 re-verifies; #4675's victim was production omantel.biz, dep 2c3f7c34) | NEVER a wipe target, NEVER a CP-2 cutover re-fire target, NEVER an in-band reclaim victim |
...
Maintenance rule: when production moves to a new env, updating this table (here + RUNBOOKS.md) is part of the promotion checklist. The W1 code protect-set (`provisioner.go`) **seeds from this repo-tracked list** and cross-checks live cloud inventory (Huawei ECS/VPC API) — never the in-memory deployments store alone.
