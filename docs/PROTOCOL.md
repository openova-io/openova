...
1. **Keystone**: `cutoverComplete=true` proven **ZERO-TOUCH** on a **fresh 2-region Huawei kom4dc Sovereign** — full 11-step chain (`platform/self-sovereign-cutover/chart`) including the 10-minute deny-egress hold against `github.com`, `ghcr.io`, `harbor.openova.io`.
2. **Then**: Agenity running on the environment + provisioning applications through Agenity with MCP integration (#4111, #4277).
3. **Then**: the ~286-row UAT ledger (`docs/ledger/UAT.md`) walked green with live evidence.
...
| W3 | ADR-0014 (sovereign-ref canon, amends ADR-0012 — 0013 is now the cross-org-denial-shape decision) + widened `check-kyverno-proxy-images.sh` | prevents registry-family recurrence | #5036, #5038, `docs/adr/` |
...
**Lane B — converged-env UAT sweep + evidence-batched closes — RUNS FIRST on the shared env.** On the named env (§2.1a; hw250 per the 2026-07-14 snapshot), walk the ⚠️ code-done rows + re-affirm the SSO re-stamp rows (58 ⚠️ + 19 SSO were the 2026-07-14 snapshot counts; re-derive the current work-list from the UAT.md head per §1.0 — live tally per PR #6690 (2026-08-25): ✅246 ❌12 ⚠️6 ⏳22 on hw305); commit stamps + evidence, then hand the `LANE-B-EXTRACTED` SHA to Lane A. Batch-close issues **with live evidence per close** (#4913 template), consuming the W5 sweep's walk-debt classification (not the raw ~54 LIKELY-DONE list). Includes #4788's 3 Governance rows (206/207/239) — the fix ALREADY shipped (PR #4789, 2026-07-05; the retrospective's "never shipped" line is the error); they are pure walk-evidence debt.
...
| The single live Sovereign | **exactly ONE deployment may exist at any time** (founder verbatim, 2026-07-15, #5111: *"You are allowed to create 1 environment at a time and that is the exact definition of flight checklist"*); as of 2026-09-02T09:40Z: **none live** (hw305 wiped 07:47Z; hw306 failed + wiped, #6786; hw307 pending — promote at `ready` + stability watch) (Step 0 re-verifies) | It IS production. NEVER an in-band reclaim/janitor victim, NEVER wiped mid-cycle. It is wiped ONLY as the explicit, verified wipe-before-fire step of the next prov cycle — and a fire while it still exists is itself the violation (#4614/#4675 were both dual-env accidents; the hw240+hw255 coexistence of 2026-07-15 was the same class) |
...
The `fire()` pre-flight gate as a decision tree — protect-list first, then refuse-don't-reclaim, then quota headroom with the wipe-release-lag poll, then the grep-auditable evidence line (the wipe endpoint carries its own extra gate, §5.2 item 5):
...
7. **Mothership image-path gate** (check 7, #6778): `prov-preflight.sh` refuses to fire unless every image a fresh Sovereign pulls resolves tag -> index -> `linux/amd64` child -> config blob along the path the NODE uses — the `registries.yaml` mirror table parsed from `infra/providers/huawei/main.tf` (on kom4dc the bastion EIP, not the public `harbor.openova.io`, which answers differently) — with the image set built from the repo (`scripts/check-mothership-proxy-image-path.sh`). Quota headroom and health endpoints say nothing about a damaged proxy-cache entry: hw306 and hw307 (2026-09-02) were fired on 13/13 green while the CNPG child manifest was 404. The stage decides the verdict, as it does for containerd — an index miss falls back upstream (WARN, fallback probed), a missing child or config behind a 200 index kills the pull (FAIL). Evidence line: `IMAGE-PATH PASS <n> images resolved through proxy-* (index+child+config)`, written next to `PRE-FLIGHT PASS`; a red result is fixed by purging the cache entry as Harbor admin, never by firing anyway.
...
| `docs/adr/` | ADR-0014: sovereign-ref canon amending ADR-0012 — "deny-egress hold is the sovereignty proof; containerd redirect is the mechanism; lint rejects only refs outside HOST_PROJECT_MAP" (#5038's ruling; 0013 is now the cross-org-denial-shape decision). The 11-step chain superseding ADR-0002's 8-Job spec is recorded via a Status note on ADR-0002 (body immutable) + STATUS.md §2.2, NOT a new ADR | Immutable, additive |
...
| `~/.claude/skills/` | Procedures, not facts. The `/close` skill now runs `gh issue close` for verified items (the agent owns the full close cycle) — the only-the-user-closes rule was repealed 2026-06-05 and the skill was updated to match | Session-local; must never contradict repo canon |
...
| C5 | Registry-pivot whack-a-mole | ADR-0014 sovereign-ref canon (amending ADR-0012); `check-kyverno-proxy-images.sh` widened to render EVERY chart in `platform/`+`products/`, fail-closed at PR time (the curated-4-chart list hid ~11 violators behind `allowExistingViolations:true`); 2-consecutive-fire recurrence of any symptom class mandates repo-wide-enumeration-first (#5010 pattern); W7 disposes of the orphaned registry-pivot WIP branch | `docs/adr/0014-*.md` + `scripts/check-kyverno-proxy-images.sh` (required check) + `docs/PRINCIPLES.md` |
...
- [ ] **An application provisioned end-to-end THROUGH Agenity**: the conversation, the resulting Application CR (`kubectl get applications.apps.openova.io -A`), and the app's HTTP 200 at its Application URL (`<orgslug>.omani.homes` canon — never openova.io) — screenshots + wire capture.
