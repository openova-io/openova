...
For pool-domain-based Sovereigns (`<sub>.openova.io`, `<sub>.omani.works`) the parent zone is delegated to the OpenOva PowerDNS NS set via the registrar (Dynadot). Each child Sovereign zone publishes its own DS in the parent zone for DNSSEC chaining. For BYO Sovereigns the operator's existing registrar publishes the NS delegation pointing at OpenOva's NS endpoints.
...
**Algorithm:** `ECDSAP256SHA256` (algorithm 13) — IETF-recommended curve for new deployments since 2014, smaller signatures than RSA, sufficient strength for the foreseeable future.

**Key model.** Each zone gets an automatically-generated KSK + ZSK pair, generated by the catalyst-dns sidecar after zone creation via `pdnsutil add-zone-key`. Keys live in the `cryptokeys` table inside the CNPG-managed `pdns-pg` Postgres database. CNPG's WAL archiving + base backup schedule (configured at the bp-cnpg level) is the disaster-recovery anchor.

**SOA serials.** `default-soa-edit-signed=INCEPTION-EPOCH` keeps SOA serials in sync across replicas after every signed-zone change. Operators don't manually bump SOA on edits.

**Rotation.** KSK rotation follows RFC 6781 / RFC 7583: add a new KSK; wait TTL on the parent DS; submit new DS to registrar; wait for new DS to propagate; mark old KSK inactive; wait one more TTL; remove old KSK. ZSK rotation is fully automated.

### 13.3 REST API

Exposed at `https://pdns.openova.io/api`, behind a Traefik basicAuth Middleware. The plaintext password is generated per-cluster (random 32 chars), bcrypt-hashed in-cluster only, and stored in K8s Secret `powerdns-api-basicauth` in the `openova-system` namespace.

Catalyst-relevant endpoints:
...
2. **cert-manager-webhook-pdns** — DNS-01 ACME challenges for wildcard certs.
...
The threshold is configurable via `.Values.dnsdist.ratelimit.qpsPerSource`.

### 13.5 Anycast deferral

The target state for `ns1.openova.io`, `ns2.openova.io`, `ns3.openova.io` is anycast Hetzner Floating IPs spread across regions. The Crossplane `XHetznerFloatingIP` composite resource definition that would allocate these does not yet exist.

**Phase-0 stand-in:** A Kubernetes `Service` of `type=LoadBalancer`. On Hetzner-managed Sovereigns the Hetzner cloud-controller-manager allocates a public IPv4 automatically; on Contabo this falls back to NodePort + external-IP routing.

**Cutover:** Once `xrd-floating-ip.yaml` + `composition-floating-ip.yaml` land in `platform/crossplane/compositions/`, bp-powerdns is bumped with `.Values.crossplane.floatingIP.enabled=true` flipped on by default.

---

## §14 — Multi-region DNS topology (lua-records)

> Source: previously `docs/MULTI-REGION-DNS.md` (merged here on 2026-05-20).

Geographic redundancy in OpenOva is realized at the **authoritative DNS** layer, not at the K8s controller layer. PowerDNS lua-records (`ifurlup`, `ifportup`, `pickclosest`, `pickrandom`, `pickwhashed`) provide:

- **Geo-aware response selection** — answer the closest healthy backend for the resolver's source IP / ECS subnet.
- **Health-checked failover** — drop a backend from the response set when a TCP/HTTP probe fails, restore when the probe recovers.
- **Latency-aware routing** — combine `ifurlup` (health) with `pickclosest` (geo) for active-active steering.
- **Same operational layer Catalyst already runs** — PowerDNS is bp-powerdns, deployed by the bootstrap kit on every Sovereign's `mgt` cluster.

This subsumes the role previously assigned to k8gb (component removed). Lua-records cover every failover scenario k8gb covered without the dedicated GSLB controller.

### 14.1 Why PowerDNS lua-records (and why not k8gb)
...
| Operator footprint | k8gb controller + CRDs + per-cluster CoreDNS pod set | None — declarative LUA records in the existing PowerDNS zone |
| Health-check primitive | k8gb-managed liveness probes | PowerDNS `ifurlup` / `ifportup` (HTTP / TCP probes from PowerDNS pods) |
| Geo selection | EdgeDNS witness + custom logic | `pickclosest`, `pickrandom`, `pickwhashed` |
...
### 14.2 Failover patterns (the lua-record cookbook)

> **Note on examples.** Backend IPv4 addresses below are placeholders. The canonical 6-record set per Sovereign zone is written by **pool-domain-manager** (PDM) on `/v1/commit`; lua-records are written by the **catalyst-dns** controller from each Application's Placement spec.
...
- PowerDNS HTTP-probes from each PowerDNS pod every 5s (configurable via `interval` option).
- `selector='all'` returns every healthy backend — the resolver's stub picks one.
- When the probe to a backend fails three times in a row, that backend is removed from the answer set within the next TTL window.
...
`pickfirst` returns the first healthy backend in the list. When primary fails the probe, the answer flips to DR within one TTL window.

**TCP-only / non-HTTP services (`ifportup`):**

```
mail.acme.com.  IN  LUA  A "ifportup(587, {'5.161.42.18', '95.217.189.42'})"
```

**Weighted round-robin (`pickwhashed`):**

```
api.acme.com.  IN  LUA  A "pickwhashed({{80, '5.161.42.18'}, {20, '95.217.189.42'}})"
```

80% of distinct client IPs are pinned to the first IP, 20% to the second (consistent hash on source IP).

### 14.3 Catalyst integration points

**Where lua-records are written.** Lua-records are part of each Sovereign's PowerDNS zone, alongside the canonical 6-record set:
...
The catalyst-dns controller reads the Application's **Placement** field from the per-Org Gitea repo, sees `placement: active-active` (or `active-hotstandby`, etc.), and synthesizes the corresponding lua-record body.

**Application Placement → lua-record selector mapping:**
...
| `active-passive-warm` | `ifurlup(..., {selector='pickfirst'})` + longer TTL (manual operator promotion) |
| `weighted-canary` | `pickwhashed({{w1, ip1}, {w2, ip2}})` — adjust weights via Catalyst console |

**Probe target.** Every Catalyst Application Blueprint MUST expose `/healthz` on its public endpoint. DNS pods probe **outbound** to the regional LB IPs over the public internet.

**Split-brain protection (failover-controller).** Lua-records are necessary but not sufficient for split-brain protection during a network partition. The failover-controller layers a lease-based witness on top (Cloudflare KV or similar — out of band from the Sovereign's own infra). Lua-record probes are the *primary* failover signal (sub-minute response); the lease becomes the *tie-breaker* for stateful promotion (OpenBao DR, CNPG primary promotion).

### 14.4 When to add a second Sovereign region
...
| Application Placement = `active-active` / `active-hotstandby` / `active-passive-warm` | Mandatory — these placements require ≥ 2 regions |
| Latency-sensitive global traffic | Strongly recommended — `pickclosest` lua-records cut median RTT |
| Cost-sensitive single-tenant Sovereign on a low-tier SLA | Defer — pay for it when a workload demands it |

The cluster name **never changes** during an upgrade from single-region to multi-region — Region A's cluster is still `hz-fsn-rtz-prod`, Region B is now `hz-hel-rtz-prod`, and neither is "primary" or "DR". Failover is a routing event, not a renaming event.

### 14.5 Operational checks
...
# Expected: current probe status (last probe timestamp, last result, current selection set).
```

---

## §15 — ClusterMesh ID assignment

> Source: previously `docs/CLUSTERMESH-CLUSTER-IDS.md` (merged here on 2026-05-20).

Every Cilium ClusterMesh peer needs a unique `cluster.name` + `cluster.id` pair. The ID is a uint8 (1–255 range; 0 is reserved) and identity collisions are silent — peer A and peer B both thinking they are id=1 produces a peering failure with no helpful log line. Per [`PRINCIPLES.md`](PRINCIPLES.md) rule #4 (never hardcode), the chart cannot ship a default cluster.id, and the IDs MUST be tracked somewhere durable. This section is that ledger.

Every per-Sovereign overlay at `clusters/<sovereign>/bootstrap-kit/01-cilium.yaml` MUST set `spec.values.cilium.cluster.name` + `spec.values.cilium.cluster.id` to a pair listed in the registry below; new Sovereigns MUST claim a fresh row in the same PR that introduces the overlay.

### 15.1 The chart shape

The `bp-cilium` umbrella chart at `platform/cilium/chart/` ships the ClusterMesh values overlay separately at `values-clustermesh.yaml`. The default `values.yaml` has no ClusterMesh block — it is opt-in per Sovereign so single-region Sovereigns don't pay the LoadBalancer / NodePort cost.
...
values:
  cilium:
    cluster:
      name: <one of the names in §15.2 below>
      id:   <matching id>
    clustermesh:
      useAPIServer: true
      apiserver:
...
### 15.3 Convention: cluster.name format
...
Examples that ARE valid: `omantel-fsn`, `omantel-hel`, `acme-nbg`, `bigtenant-use1`. NOT: `omantel.omani.works` (uses the full FQDN — too long, dots not allowed), `prod-fsn` (no sovereign tied), `OMANTEL-FSN` (case-sensitive on cluster identity).

### 15.4 Mesh boundaries
...
When Continuum goes live, the cross-Sovereign management plane mesh (`hz-nbg-mgt-prod` ↔ data-plane peers) will form a SECOND mesh graph. Allocate cluster.ids in the same table but mark the mesh column distinctly so the duplicate-id check above keeps working.

### 15.5 Operator runbook (per-peer bringup)
...
- [`RUNBOOKS.md`](RUNBOOKS.md) — bringing a Sovereign online (Phase 0 / 1) + writing Blueprints + Crossplane Compositions for advanced authors.
