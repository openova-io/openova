...
| **3. 2 independent CNPG clusters + region-kill failover** | 🟡 INFRA-READY / WALK-BLOCKED-ON-V15 | D31 (#1831) chart-render verified previously (`bp-cnpg-pair` emits primary+replica CRs). End-to-end region-kill walk has never run on a tenant org. | Blocks on Pillars 1+2 reaching the wizard-finish step on a fresh prov. |
