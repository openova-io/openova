...
| 7 | **Phase-1 image path through the mothership proxy** (#6778) | `HW_TFVARS=… scripts/check-mothership-proxy-image-path.sh` (run by `prov-preflight.sh` check 7): every repo-derived Phase-1 image (rendered bootstrap-kit charts + CNPG pins + cloud-init flux/cilium) walked tag -> index -> `linux/amd64` child -> config blob along the node's OWN mirror path, parsed from the `registries.yaml` heredoc in `infra/providers/huawei/main.tf` (bastion EIP on kom4dc — NOT `harbor.openova.io`, which answers differently) | `IMAGE-PATH PASS <n> images resolved through proxy-* (index+child+config)`. A health endpoint or `/v2/` ping is NOT this check — both answered 200 on 2026-09-02 while the CNPG amd64 child was 404 (hw306/hw307 burned). A missing child/config behind a 200 index is fatal (no containerd fallback) = FAIL; a clean index miss is WARN with the upstream fallback probed. Red = purge the damaged proxy-cache entry as Harbor **admin**, re-run |
