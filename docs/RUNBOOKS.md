...
| Current production Sovereign | **none live as of 2026-09-02T09:40Z** — hw305 wiped 07:47Z (train #6778); hw306 (dep `e68e79721ecbde62`) failed at tofu apply 09:10Z (#6786) and was wiped 09:24Z; **hw307 pending** — promote this row only when hw307 holds `ready` through the 60-min #6485 stability watch, pasting the `GET /deployments/<id>` `status:"ready"` output in the promotion PR (re-verify against live inventory before acting; #4675's victim was production omantel.biz, dep `2c3f7c34`) | NEVER a wipe target, NEVER a converged-env cutover re-fire target (§0.5), NEVER an in-band quota-reclaim victim |
...
flowchart TD
  ...
  NoReset --> Gate["Run prov-preflight.sh<br/>incremented FQDN · zero active deployments<br/>EIP-orphan headroom · no-foreign-Sovereign gate<br/>Phase-1 image path through the mothership proxy"]
...
2. **Do NOT reset UAT evidence here — this step runs AFTER the new env is live.** Its old position, and its old description as a flush, are both stale:
   ...
   - Running it **before** the fire stamps the ledger header with an environment that does not exist yet, so `scripts/uat-drift-guard.py` resolves `current env: NONE/pending` and **every predecessor-cited green becomes stale at once**. Measured 2026-08-14 in the hw296→hw297 gap: `origin/main` PASSED the guard (39 rows carried with the scheduler's consent) while the pre-fire reset FAILED it with **105 greens stranded**. PR #6330 was closed unmerged for exactly this.
   - The guard's own remediation text says it directly: do not run `reset-uat.py` to quiet it — carry-forward re-marks those rows `PENDING`, which parks the work rather than doing it and drops each row out of the headline count. That demotion took 225 greens to 178 in PR #6250.

   **Correct order:** fire → env reaches `ready` → `python3 scripts/reset-uat.py hw<NNN>` → `python3 scripts/uat-confidence.py --due --env hw<NNN>` for the walk-list. The scheduler decides scope; re-walking all 286 is the treadmill (measured on hw297: 172 due, 108 skipped at high confidence).
3. **Run the pre-flight gate** — `scripts/prov-preflight.sh <FQDN> <BUCKET> [<qaTestEnabled>] [<fireCutoverOnHandover>]` with `COOK` (owner-scoped `catalyst_session` cookie) and `HW_TFVARS` (path to a `tofu.auto.tfvars.json` carrying Huawei AK/SK and `harbor_robot_token`) exported. It checks: incremented FQDN, zero active deployments, EIP-orphan headroom, the ground-truth **no-foreign-Sovereign** gate (live Huawei ECS query — the #4675 guard), and the **Phase-1 image path through the mothership proxy** (check 7). Confirm §0.2 headroom (VPC ≥2 free, EVS ≥100 free) as part of the same pass. **Non-zero exit = NO fire.** If headroom is missing after a wipe, poll until the ~15-min wipe-release lag clears — never fire on assumed headroom.
   - **Check 7 walks every image a fresh Sovereign pulls, along the path the node actually pulls it** (`scripts/check-mothership-proxy-image-path.sh`, #6778). The image set is built from the repo — every rendered bootstrap-kit chart (`check-freshprov-image-registry.sh --list-images`), the CNPG postgres + cloudnative-pg controller pins, the cloud-init Phase-0 flux/cilium pulls — and each ref is resolved through the `registries.yaml` mirror table parsed out of `infra/providers/huawei/main.tf`, then fetched as the kubelet would: tag -> OCI index -> `linux/amd64` child manifest -> config blob. **Probe the face the node uses, not the public one:** on kom4dc that is the bastion EIP (`http://212.72.24.20:5000` / `:5002`), the mothership Harbor's NAT-bypass face (#3913). Measured 2026-09-02: a child manifest that 404s on `https://harbor.openova.io` is 200 on the face a node pulls through, so probing the public host reports ~30 phantom failures. The verdict is stage-dependent, because containerd is: a clean **index** 404 falls back to the upstream registry (reported WARN, and the fallback is probed — a pin gone from upstream too is a FAIL), while a missing **child or config behind a 200 index** kills the pull outright with no fallback and hard-FAILS the gate. That second shape is the incident: the 2026-09-01 registry blob-store wipe (memory `reference_mothership_harbor_retention_filled_disk_never_wipe_registry`) left the proxy-cache manifest rows intact, so `/api/v2.0/health`, the `:5000`/`:5002` pings and even `GET …/postgresql/manifests/16` all answered 200 while the amd64 child was 404 — and hw306 + hw307 were fired into it (every `shared-pg-*-initdb` Pod ImagePullBackOff, Phase 1 timed out at 2 h 40 min, keycloak never installed — #6778, #6795). **Remediation for a red check 7:** purge the damaged proxy-cache entry as Harbor **admin** (the robot gets 401 on delete) so the next pull re-fetches from upstream, re-run the gate, and fire only on `IMAGE-PATH PASS <n> images resolved through proxy-* (index+child+config)`. Paste that line into the train manifest next to the `PRE-FLIGHT PASS` line. First live run, 2026-09-02: **115 images in 50 s**, and it caught a real pin — `harbor.openova.io/proxy-dockerhub/alpine:3.20` in bp-openbao served index 200 / child 200 / config **404** (Harbor keeps Docker Hub blobs under the wire path `library/alpine`), fixed in the same change.
...
**Phase 1 timed out but the Sovereign converged on its own (#6795 / #6799).** The record reads `status=failed`, `phase1Outcome=timeout`, yet both clusters are alive, the bootstrap-kit HelmReleases are Ready and `https://console.<sovereign-fqdn>/` answers 200 (hw307, 2026-09-02). When the cloud is intact and only convergence was late, use the re-watch: `POST /api/v1/deployments/{id}/phases/phase1-watch/retry`. It re-attaches only the HelmRelease watcher (no `tofu`, no cloud writes) and, on convergence, flips the record to `ready` and fires the handover exactly as the first-run watch does. Do not re-run `tofu apply` (a Phase-0 retry) on a converged Sovereign: on kom4dc the NAT-EIP rotation leaves region B's SNAT rule + EIP outside tofu state, so the re-apply plans additions and fails `VPC.20xx` while the record stays `failed`. The re-watch answers 409 with the reason when a watch is already attached, the record is adopted, Phase 0 produced no result, or no kubeconfig exists on the PVC.
...
- `scripts/prov-preflight.sh` — §0.3 mandatory pre-fire gate (VPC/EIP/foreign-Sovereign checks + check 7, the Phase-1 image path through the mothership proxy)
- `scripts/check-mothership-proxy-image-path.sh` — §0.3 check 7 standalone: walks every repo-derived Phase-1 image tag → index → amd64 child → config blob through `harbor.openova.io` with the robot pull token; `--self-test` runs it against a local fake registry (#6778)
