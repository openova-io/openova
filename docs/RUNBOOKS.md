...
If any line is missing, see [`ARCHITECTURE.md`](ARCHITECTURE.md) §13 (PowerDNS deployment shape) for the per-Sovereign zone model.
...
| HelmRelease: `failed to authorize: 401 Unauthorized | ghcr.io/openova-io/bp-cilium` | GHCR token expired or wrong scope | Rotate per [`SECURITY.md`](SECURITY.md) §11 (Secret rotation); rotate `flux-system/ghcr-pull` Secret on existing Sovereigns + `flux reconcile source helmrepository` |
...
- [`ARCHITECTURE.md`](ARCHITECTURE.md) — Catalyst target architecture (incl. PowerDNS deployment, multi-region DNS, ClusterMesh ID registry)
...
- [`SECURITY.md`](SECURITY.md) — identity, secrets, rotation (incl. GHCR pull token, Dynadot credentials, Hetzner tokens)
- [`BUSINESS-STRATEGY.md`](BUSINESS-STRATEGY.md) — franchise model + voucher mechanism + product families
