...
### 9.2 Wipe → pre-flight → FIRE (2026-09-02, UTC)

- **WIPED hw305** — `POST /sovereign/api/v1/deployments/b2b00ce4c833badf/wipe` body `{}` → **HTTP 202** at 07:33:24Z (`status:"wiping"`); record **`wiped` at 07:47:13Z**; `lastWipeReport.providerPurge` removed floating IPs 212.72.24.1 / .74 / .35 / 5.37.79.43 / 212.72.24.25 (incl. the 3 orphan `nat-preflight` EIPs), both ELBs, both VPCs, both subnets, the OBS bucket; `tofuDestroyed:false` (provider purge path).
- **ZERO-REMNANT verified** by signed GET at 08:51:50Z: quotas **vpc 1/5 · publicIp 1/10 · subnet 3/100 · loadbalancer 0/50**; ECS 1 (bastion) · VPC 1 (bastion) · EIP 1 (`212.72.24.20` only) · NAT 0 · ELB 0 · EVS 2 (bastion) — exactly the expected post-wipe baseline from §9.1. Cooldown since `wiped`: 74 min.
- **T-0 pin re-verification** on origin/main `9e6049273`: `check-bootstrap-kit-pin-sync.sh --check-ghcr` PASS (68/68) · `check-train-coherent.sh --ref origin/main` **COHERENT — the train is safe to fire** · last 3 Blueprint Release runs success · in-flight CI: only main's privacy/merge-conflict guards.
- **PRE-FLIGHT PASS** vpc=1/5 evs=2/400 eip=bastion-only obs=48/100 — all 13 checks ✅ at **09:00:48Z** (`scripts/prov-preflight.sh hw306.omani.works catalyst-hw306-omani-works false false`, BEARER = owner RS256 minted 08:5xZ, HW_TFVARS = hw305 tfvars copy); 4b ✅ on `b27d54a`; 0 other active deployments; no foreign ECS.
- **FIRED 2026-09-02T09:01:03Z**: `POST /sovereign/api/v1/deployments` → **HTTP 201** `{"id":"e68e79721ecbde62","status":"provisioning"}`; body as §5 with **`fireCutoverOnHandover:false`** (2-region me-east-215-a/-b, cp m7n.xlarge.8, 5× m7n.2xlarge.8 per region, evs-ssd, active-hotstandby, marketplace + console isolation on, qaTestEnabled false, omani.works primary + omani.homes/omani.rest org-pool). Record: `e68e79721ecbde62 provisioning hw306.omani.works startedAt 2026-09-02T09:01:05Z`. Fire 1 of max 2 today.
- **Merge freeze in force** from 07:33Z (wipe POST) until `cutoverComplete=true`: no `core/**`, `products/catalyst/bootstrap/**`, `products/catalyst/chart/**`, `infra/providers/**` merges. Docs/scripts-only PRs remain mergeable.
