...
| Partners (`partners.manage`; a partner owner holds `partner.self.manage` on its own) | `GET/POST /partners` · `GET/PATCH/DELETE /partners/{id}` (the delete is refused, naming what still depends on the partner) · `GET/POST /partners/tiers` · `PUT /partners/tiers/{id}/discounts` · `DELETE /partners/tiers/{id}` (refused while a partner is on the tier) · `PUT /partners/{id}/retail-rule` (re-derives; the response lists the below-buy lines) · `GET /partners/{id}/retail-book` · `GET /partners/{id}/customers` · `GET /partners/{id}/statements` · `GET /partners/{id}/margin?period=` · `GET /partners/{id}/account` · `GET/POST /partners/{id}/users` · `DELETE /partners/{id}/users/{email}` · `PATCH /customers/{id} {partner_id}` |
