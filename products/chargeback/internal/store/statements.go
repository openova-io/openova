...
// partner it belongs to (DESIGN.md §Partners).
...
type StatementDraft struct {
	...
	DiscountRule string
	// The partner keys (DESIGN.md §Partners). PartnerID is the customer's
...
// §Partners) — newest period first. The operator scope reads them all.
...
func (s *Store) issueStatementOnce(ctx context.Context, id, actor string) (st Statement, transitioned bool, err error) {
	...
	default:
		...
		case kind == StatementKindCommission && ratOf(Decimal(total)).Sign() > 0:
			...
			// §Partners, agent model): a CREDIT on the party's ledger, with
