...
type Customer struct {
	...
	// PartyKind says what this row IS (DESIGN.md §Partners): a customer, or
...
type PriceBook struct {
	...
	// A partner's DERIVED retail book (DESIGN.md §Partners): owned by the
...
type Discount struct {
	...
	CreatedAt time.Time `json:"created_at"`
	// TierID makes this a PARTNER TIER discount (DESIGN.md §Partners): a
...
type Statement struct {
	...
	// The partner keys (DESIGN.md §Partners), all additive. PartnerID is the
...
type RatedLine struct {
	...
	// The partner waterfall per line (DESIGN.md §Partners), set only on the
...
const (
	...
	// The two partner roles (DESIGN.md §Partners), bound at one partner: an
...
type RoleBinding struct {
	...
	// PartnerID is set on a partner-scoped binding (DESIGN.md §Partners).
...
// PARTNER principal (DESIGN.md §Partners) sees the SET of customers assigned to
