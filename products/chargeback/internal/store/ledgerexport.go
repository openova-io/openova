...
type JournalEvent struct {
	...
	TaxRules      []TaxLine       `json:"tax_rules,omitempty"`
...
// snapshotTaxEntry is one entry of a frozen tax snapshot's per-rule summary.
// The rule engine (DESIGN.md §17) writes `rule_id`/`rule_name`; `code`/`name`
// are accepted so a snapshot frozen by an earlier build, or by a reader that
// used the shorter names, still names its rules in the journal instead of
// posting anonymously.
type snapshotTaxEntry struct {
...
func taxRulesOf(snapshot []byte, tax Decimal) []TaxLine {
	if isZeroDec(tax) {
		return nil
	}
	single := []TaxLine{{Tax: tax}}
	if len(snapshot) == 0 {
		return single
	}
	var raw struct {
		Rate  Decimal `json:"rate"`
		Rules []snapshotTaxEntry `json:"rules"`
		Lines []snapshotTaxEntry `json:"lines"`
	}
	...
	out := make([]TaxLine, 0, len(entries))
	for _, e := range entries {
		t := e.Tax
		if strings.TrimSpace(string(t)) == "" {
			t = e.Amount
		}
		out = append(out, TaxLine{
			RuleID:   firstNonEmpty(e.RuleID, e.Code),
			RuleName: firstNonEmpty(e.RuleName, e.Name),
			Kind:     e.Kind,
			Category: e.Category,
			Rate:     e.Rate,
			Base:     e.Base,
