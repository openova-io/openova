...
func allocateTx(ctx context.Context, tx *sql.Tx, customerID, statementID string, paymentID int64, creditNoteID string, amount *big.Rat, actor string, at time.Time) error {
	...
	// Judged at the currency's minor unit (DESIGN.md §8.6): an allocation
	// that overshoots what is owed by less than half a unit is the
	// settlement the dialog offered, and is clipped to the exact balance so
	// allocations never exceed the invoice; half a unit or more is refused.
	var currency string
	if err := tx.QueryRowContext(ctx, `SELECT currency FROM statements WHERE id = $1`, statementID).Scan(&currency); err != nil {
		return mapErr(err)
	}
	if over := new(big.Rat).Sub(outstanding, amount); overpaysAt(over, currency) {
