...
// OutstandingAt is total − payments − credit notes, exactly.
...
func lockStatement(ctx context.Context, tx *sql.Tx, id string) (status string, total, paid Decimal, due *time.Time, err error) {
	...
	err = tx.QueryRowContext(ctx, `SELECT st.status, st.total::text, st.due_at,
		...
		FROM statements st WHERE st.id = $1 FOR UPDATE`, id).Scan(&status, &t, &d, &p, &c)
	if err != nil {
		return "", "", "", nil, mapErr(err)
	}
	return status, Decimal(t), addDec(Decimal(p), Decimal(c)), timePtr(d), nil
}
...
func (s *Store) RecordStatementPayment(ctx context.Context, id string, in PaymentInput) (st Statement, p StatementPayment, err error) {
	...
	outstanding := new(big.Rat).Sub(ratOf(total), ratOf(paid))
	if in.Status == PaymentReceived && amt.Cmp(outstanding) > 0 {
		return Statement{}, p, fmt.Errorf("%w: payment of %s exceeds the outstanding balance of %s", ErrConflict, decOf(amt), decOf(outstanding))
	}
	var customerID, currency string
	if err := tx.QueryRowContext(ctx, `SELECT customer_id, currency FROM statements WHERE id = $1`, id).Scan(&customerID, &currency); err != nil {
		return Statement{}, p, mapErr(err)
	}
	var paymentID int64
	...
	if in.Status == PaymentReceived {
		...
		// allocation, no ledger credit, and the invoice cannot flip.
