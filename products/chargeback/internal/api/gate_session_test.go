...
func TestGateIdentitySignsInWithoutASecondPrompt(t *testing.T) {
	...
	if got := getWithHeader(t, h, "/api/v1/auth/me", "X-Forwarded-Email", opEmail); got != 200 {
		t.Fatalf("operator identity from the gate: got %d, want 200 (a signed-in user was asked to sign in again)", got)
	}
	if got := getWithHeader(t, h, "/api/v1/customers", "X-Forwarded-Email", opEmail); got != 200 {
...
func TestOnlyTheConfiguredHeaderIsTrusted(t *testing.T) {
	...
	// X-Auth-Request-Email is oauth2-proxy's RESPONSE header; it must never
	// be honoured as an upstream identity just because it looks official.
...
func TestGateIdentityWithoutAGrantIsNotAdmitted(t *testing.T) {
	...
	if got := getWithHeader(t, h, "/api/v1/customers", "X-Forwarded-Email", "stranger@example.com"); got != 401 {
...
func TestGateHeaderRejectsNonEmailValues(t *testing.T) {
	...
	for _, v := range []string{"", "   ", "not-an-email", "admin"} {
		if got := getWithHeader(t, h, "/api/v1/customers", "X-Forwarded-Email", v); got != 401 {
