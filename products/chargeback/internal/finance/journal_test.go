...
func invoiceEvent() store.JournalEvent {
	return store.JournalEvent{
		...
		TaxRules:      []store.TaxLine{{Rate: "0.05", Tax: "50.000000"}},
	}
}
...
func TestMultiRateTaxEmitsOnePayableLinePerRule(t *testing.T) {
	...
	ev.TaxRules = []store.TaxLine{
		{RuleID: "VAT-5", RuleName: "VAT standard", Rate: "0.05", Tax: "40.000000"},
		{RuleID: "EXCISE", RuleName: "Excise", Rate: "0.15", Tax: "15.000000"},
	}
	b := emit(t, ev)
	payable := 0
	for _, l := range b.Lines {
