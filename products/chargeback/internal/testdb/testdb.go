...
const wipeSQL = `TRUNCATE TABLE audit_log, sessions, pins, invites, rated_lines, invoice_allocations, account_entries, credit_notes, credit_note_sequences, payment_intents, collection_reminders, customer_suspensions, payments, commercial_outbox, statements, invoice_sequences, usage_records, resource_inventory, cost_sources, credentials, role_bindings, group_role_mappings, discounts, budgets, budget_alerts, saved_views, report_deliveries, report_schedules, currency_rates, customers, price_items, price_books, capacity_pool_history, capacity_pools, sku_caps, capacity_zones, capacity_regions, estimates, contract_items, contracts, partner_retail_rules, partners, partner_tiers,
finance_journal_lines, finance_periods, finance_reconciliation_lines, finance_reconciliations, account_mappings RESTART IDENTITY CASCADE;
...
ON CONFLICT (id) DO UPDATE SET discount_rule = DEFAULT, invoice_prefix = DEFAULT, commercial_provider = DEFAULT, external_ingest = DEFAULT, tax_rate = DEFAULT, tax_registration_number = DEFAULT, legal_name = DEFAULT, address = DEFAULT,
  credit_note_prefix = DEFAULT, reminder_days = DEFAULT, escalation_days = DEFAULT, escalation_action = DEFAULT, public_price_book_id = DEFAULT, updated_at = now()`
