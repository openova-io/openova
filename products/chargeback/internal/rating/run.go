...
func Run(ctx context.Context, st *store.Store, period, customerID string) ([]Result, error) {
	...
	for _, c := range customers {
		...
		stmt, detail, err := rateCustomer(ctx, st, c, from, to)
		if err != nil {
...
func rateCustomer(ctx context.Context, st *store.Store, c store.Customer, from, to time.Time) (store.Statement, rateDetail, error) {
