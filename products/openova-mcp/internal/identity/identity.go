...
type Resolver struct {
	...
	// rsaPubs is the SET of RS256 public keys this instance trusts. A
	// Sovereign does not have one session signer, it has several, and pinning
	// exactly one refuses genuine bearers at authentication:
	//
	//   - the MOTHERSHIP-injected handover key lives in
	//     `catalyst-handover-jwt-public`, and #4450's decidePubkeyPublish
	//     deliberately PRESERVES it on a Sovereign — overwriting it 401s every
	//     inbound owner handover. That Secret is this chart's default verify
	//     source (#5167).
	//   - every session bearer a caller actually presents is signed by the
	//     Sovereign catalyst-api's OWN handoverjwt signer, self-generated by
	//     LoadOrGenerate (HandlePinVerify, org_handover, org_enter_org,
	//     seedMCPBearer all route through SignCustomClaims) — and its key file
	//     sits on a per-Pod RWO PVC, so a multi-region Sovereign has one such
	//     signer per catalyst-api.
	//
	// catalyst-api hit this first and fixed it the same way: auth_handover.go
	// builds a CANDIDATE list (#4101 — "the keyfunc tries the mounted key
	// first, then the local one") after a single pinned key rejected every
	// Enter-org token with `crypto/rsa: verification error`. #5175 filed the
	// same option for the MCP; UAT rows 212/213 on hw293 are what it costs
	// unimplemented — an Org-scoped bearer the same catalyst-api accepts
	// (whoami 200, orgScoped:true) refused here 3/3.
	//
	// This is a set of REAL public keys, not a relaxation: a bearer must still
	// verify against one of them, and an empty set refuses everything.
...
func NewRS256Resolver(pub *rsa.PublicKey, pinnedCtx Context) *Resolver {
	return NewRS256SetResolver([]*rsa.PublicKey{pub}, pinnedCtx)
}

// NewRS256SetResolver builds a resolver that accepts a bearer verifying against
// ANY key in pubs — the Sovereign's full signer set (see Resolver.rsaPubs for
// why one key is never enough). Keys are tried in order; the first that
// validates wins. A nil/empty set REFUSES every bearer: an unwired key source
// must never degrade into "skip verification".
func NewRS256SetResolver(pubs []*rsa.PublicKey, pinnedCtx Context) *Resolver {
	trimmed := make([]*rsa.PublicKey, 0, len(pubs))
	for _, p := range pubs {
		if p != nil {
			trimmed = append(trimmed, p)
		}
	}
	return &Resolver{mode: VerifyRS256PublicKey, rsaPubs: trimmed, pinnedCtx: pinnedCtx}
}

// RS256KeyCount reports how many verify keys this resolver holds. Used for the
// startup log line so an operator can see the set is non-trivial without any
// key material being printed.
func (r *Resolver) RS256KeyCount() int { return len(r.rsaPubs) }
...
func (r *Resolver) Resolve(rawBearer string) (*Identity, error) {
	...
	case VerifyRS256PublicKey:
		// Try each trusted key in turn. jwt.ParseWithClaims short-circuits on
		// whatever single key the keyfunc returns, so — exactly as
		// catalyst-api's auth_handover.go does for the same reason (#4101) —
		// the parse is attempted once per candidate and the first that
		// validates wins. The claims struct is reset between attempts so a
		// partially-populated failure cannot leak into the accepted identity.
		//
		// An empty set falls straight through to the error below: no key, no
		// acceptance. Never a skipped verification.
		if len(r.rsaPubs) == 0 {
			err = errors.New("no RS256 verify key is configured")
		}
		for i, pub := range r.rsaPubs {
			attempt := &sharedauth.Claims{}
			_, perr := parser.ParseWithClaims(raw, attempt, func(t *jwt.Token) (any, error) {
			...
			// Keep the LAST failure as the reported reason. An expired or
			// malformed token fails identically against every key, so the
			// message an operator sees is unchanged in that (common) case.
			err = perr
			if i == len(r.rsaPubs)-1 && len(r.rsaPubs) > 1 {
				err = fmt.Errorf("%w (tried %d trusted RS256 keys)", perr, len(r.rsaPubs))
			}
		}
