...
type pinIssueResponse struct {
	...
	// Pin is the sign-in code, echoed in the response ONLY on a Sovereign
	// that has explicitly opted in via CATALYST_PIN_ECHO=true. `omitempty`
	// means the field is absent from the JSON entirely when off, so a
	// production response is byte-identical to before this field existed.
	// See prelivePinEcho.
	Pin string `json:"pin,omitempty"`
}

// prelivePinEcho reports whether this Sovereign may return the sign-in code in
// the /pin/issue response body instead of only mailing it.
//
// WHY THIS EXISTS. The funnel end-to-end walk — the one that creates a customer
// Organization, which UAT rows 5/6/10/11 need to exist before they can be
// evaluated at all — stops dead at /checkout/, which requires a 6-digit code
// delivered by email. On a pre-live Sovereign there is frequently no reachable
// mailbox: measured on hw292 (#5799), mail/stalwart/webmail all answer 404 on
// every retrieval path and 993/587/8080 are closed, while the kube-apiserver
// route to the pin store times out. The result is that a walk which is
// otherwise fully automatable requires a human with a mailbox, so it does not
// run in CI and the rows stay unwalked indefinitely.
//
// SAFETY. This is deliberately hostile to accidental enablement:
//
//   - Default is OFF. The env var absent, empty, "false", "0", "no" or any
//     unrecognised value all mean off. Only the exact string "true"
//     (case-insensitive, trimmed) turns it on — parsed with strconv.ParseBool
//     and then re-checked, so a typo like "ture" fails closed rather than
//     tripping a loose truthiness test.
//   - It changes only the RESPONSE. The PIN is still generated by crypto/rand,
//     still stored with the same TTL, and still mailed. Nothing about issuance
//     or verification weakens, so a Sovereign with the flag on is not easier to
//     brute-force than one without.
//   - `omitempty` keeps the wire format unchanged when off.
//
// This must never be set on a Sovereign serving real customers. It is for
// pre-live environments whose whole purpose is to be walked.
func prelivePinEcho() bool {
	raw := strings.TrimSpace(os.Getenv("CATALYST_PIN_ECHO"))
	if raw == "" {
		return false
	}
...
func (h *Handler) HandlePinIssue(w http.ResponseWriter, r *http.Request) {
	...
	resp := pinIssueResponse{
		OK:           true,
		Sent:         true,
		RequestID:    requestID,
		ExpiresInSec: int(pinTTL.Seconds()),
	}
	// Pre-live only, strictly opt-in — see prelivePinEcho. Logged at WARN so an
	// operator who set this by accident finds it in the ordinary log stream
	// rather than discovering it from a leaked code.
	if prelivePinEcho() {
		resp.Pin = pin
		h.log.Warn("pin/issue: CATALYST_PIN_ECHO is enabled — the sign-in code is being returned in the response body; this must NOT be set on a Sovereign serving real customers",
			"email", email, "requestID", requestID)
	}
