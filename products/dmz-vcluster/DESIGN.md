...
catalyst-controllers, and Keycloak, a compromise of any Organization Pod
becomes a beach-head into the management plane.

## Solution

A DMZ vCluster — an isolated virtual Kubernetes cluster running INSIDE
the management cluster's host namespace. Organization workloads land in the
...
NetworkPolicy boundary; and the Organization's only path to the public
...
[host:dmz openova-system] ◄─── default-deny + allow-essentials
       ...
       │
       └─ Organization Pods (own Cilium identity)
              │
...
| Storage | the Sovereign's block-storage StorageClass PVC for vCluster etcd (no shared PV with management) |
| Auth | Separate Keycloak realm OR realm role per Organization model (sovereign-admin picks per overlay) |
...
`products/`. NetBird (which is sovereign-admin-facing) lives under
`platform/`. DMZ vCluster is fundamentally an Organization-facing primitive —
the customer chooses to run their workload in a DMZ vCluster, the
sovereign-admin chooses to run NetBird.
...
- Cilium ClusterMesh per-Organization pinning: today the DMZ vCluster runs
  on a single Sovereign; multi-region active-hotstandby for DMZ
  Organizations is an EPIC-6 follow-up.
- Per-Organization Keycloak realm provisioning automation (sovereign-admin
  manually provisions today via the bp-keycloak realm-config
  ConfigMap pattern; full automation lives in a future controller).
- Organization-side observability stack (own Hubble UI inside the
  vCluster) — sovereign-admin opts in via a per-Organization overlay alongside
