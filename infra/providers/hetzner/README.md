...
| `hcloud_load_balancer.main` (`lb11`) | Public IPv4 in the primary region; forwards 80→80, 443→443 (to the Cilium Gateway Service type=LoadBalancer — hostNetwork off, no NodePort; #4682) and 53→53 (powerdns-anycast Service type=LoadBalancer, `allocateLoadBalancerNodePorts: false` — §854 / #4765: NodePorts are ABSOLUTELY FORBIDDEN). The `53→53` forward is §854-clean and **LIVE since [#5086](https://github.com/openova-io/openova/issues/5086)/[#5090](https://github.com/openova-io/openova/issues/5090)** (2026-07-15). It was previously inert — the anycast Service takes its VIP from Cilium LB-IPAM (gated off on Hetzner) and hcloud-ccm cannot materialise it (no nodePort/location annotation), so node:53 did not bind. The fix is the bp-powerdns ≥1.2.21 `dnsdist.hostPortMode` DaemonSet, which binds **hostPort:53** (never a nodePort) on every node; `infra/providers/_shared/cloudinit-control-plane.tftpl` sets `POWERDNS_DNSDIST_HOSTPORT: "true"` unconditionally in the `provider == "hetzner"` branch, so the DNS front door is on by default for every Hetzner prov and needs no operator action. Historical note: node:30053 was equally dead since the bp-powerdns 1.2.18 §854 flip. |
