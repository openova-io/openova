...
| **Provision fresh Sovereign** | **(0) 🛑 UAT carry-forward runs AFTER the env is live, NOT before the fire.** `scripts/reset-uat.py` has been carry-forward (not flush) since 2026-08-11, and running it pre-fire stamps the header with an env that does not exist yet, so `uat-drift-guard.py` strands every predecessor-cited green. Measured 2026-08-14: main PASSED the guard (39 carried) while the pre-fire reset FAILED it with 105 greens stranded — PR #6330 closed unmerged for exactly this. Correct order: fire → `ready` → `reset-uat.py <env>` → `uat-confidence.py --due --env <env>` for the walk-list (#3132, superseded on ordering). (1) `gh api /repos/openova-io/openova/packages/container/<bp-*>/versions` for active chart pins. (2) Pick `parent_domains_yaml` TLD per L3 rotation. (3) POST `/sovereign/api/v1/deployments` with auth (handover JWT from `/deps/handover-jwt-private.pem`). |
